...
public class CustomerTypesService implements ICustomerTypesService {
	...
	public int selectAll(List<CustomerTypes> list) {
		...
		try {
			...
			root.fetch("customer_channel", JoinType.LEFT);
			cq.select(root);
	...
	public int search(String json, List<CustomerTypes> list) {
		...
		try {/* { customer_types_info:{code:'',name:''}} */
			...
			root_.fetch("customerGroup", JoinType.LEFT);
			...
			dis1.getExpressions().add(
					cb.like(cb.function("replace", String.class, cb.lower(root_.get("name")), cb.literal("đ"),
							cb.literal("d")), pNameLike));
			predicates.add(dis1);
	...
	public int checkCustomerTypeCode(String code, long customerTypesId) {
		...
		try {
			...
			Root<CustomerTypes> root = cq.from(CustomerTypes.class);
			cq.select(cb.count(root.get("id"))).where(cb.equal(root.get("code"), code),
					cb.notEqual(root.get("id"), customerTypesId));
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public CustomerTypes findByCode(String code) {
		int res = -1;
		try {
			...
			e.printStackTrace();
			logger.error("CustomerTypesService.findByCode:" + e.getMessage(), e);
		}
		return null;
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public CustomerTypes findByName(String name) {
		int res = -1;
		try {
			if (name != null && !"".equals(name)) {
				...
				TypedQuery<CustomerTypes> query = em.createQuery(cq);
				List<CustomerTypes> customerTypes = query.setMaxResults(1).getResultList();
				if (customerTypes.size() != 0)
					return customerTypes.get(0);
			}
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("CustomerTypesService.findByName:" + e.getMessage(), e);
		}
		return null;
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public int complete(String text, List<CustomerTypes> list) {
		int res = -1;
		try {
			if (text != null) {
				CriteriaBuilder cb = em.getCriteriaBuilder();
				...
				cq.select(cb.construct(CustomerTypes.class, root.get("id"), root.get("code"), root.get("name"))).where(
						dis);
				TypedQuery<CustomerTypes> query = em.createQuery(cq);
				list.addAll(query.getResultList());
			}
			...
			logger.error("CustomerTypesService.complete<Text,list>:" + e.getMessage(), e);
		}
		return res;
	}
}
