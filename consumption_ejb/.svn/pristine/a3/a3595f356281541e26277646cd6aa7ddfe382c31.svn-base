...
public class XacNhanKiemTraDetailService {
	...
	public List<XacNhanKiemTraDetailDTO> findByXacNhanKiemTraDTO(long importId) {
		try {
			...
			cq.select(
					cb.construct(XacNhanKiemTraDetailDTO.class, root_.get("id"),product_.get("product_code"),
							product_.get("product_name"), product_.get("unit"),
							product_.get("packing_unit"), product_.get("specification"),
							root_.get("quantityCa1"), root_.get("quantityCa2"), root_.get("quantityCa3"), root_.get("quantityTTSP"),
							root_.get("chenhlech"),root_.get("trangthaicl"),root_.get("dataPallet"),root_.get("note"))).where(cb.equal(root_.get("xacNhanKiemTra").get("id"), importId));
