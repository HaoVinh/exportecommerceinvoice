...
public class PaymentMethodService implements IPaymentMethodService {
	...
	public PaymentMethod findById(long id) {
		try {
			...
			logger.error("PaymentMethodService.findById:" + e.getMessage(), e);
		}
		return null;
	}
	...
	@Override
	public PaymentMethod selectByCode(String code) {
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<PaymentMethod> cq = cb.createQuery(PaymentMethod.class);
			Root<PaymentMethod> root = cq.from(PaymentMethod.class);
	...
	@Override
	public PaymentMethod selectByName(String name) {
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<PaymentMethod> cq = cb.createQuery(PaymentMethod.class);
			Root<PaymentMethod> root = cq.from(PaymentMethod.class);
			cq.select(root).where(cb.equal(root.get("method_name"), name));
			TypedQuery<PaymentMethod> query = em.createQuery(cq);
			return query.getSingleResult();
		} catch (Exception e) {
			// logger.error("PaymentMethodService.selectByCode:"+e.getMessage(),e);
	...
	public String initCode() {
		try {
			...
			cq.multiselect(cb.coalesce(cb.max(cb.quot((Expression) cb.substring(root.get("method_code"), 3), 1)), 0));
	...
	@Override@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public int complete(String text, List<PaymentMethod> list) {
		int res = -1;
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<PaymentMethod> cq = cb.createQuery(PaymentMethod.class);
			Root<PaymentMethod> root = cq.from(PaymentMethod.class);
			Predicate dis = cb.disjunction();
			dis.getExpressions().add(cb.equal(root.get("method_code"), text));
			dis.getExpressions().add(cb.like(root.get("method_name"), "%" + text + "%"));
			cq.select(root).where(dis);
			TypedQuery<PaymentMethod> query = em.createQuery(cq);
			list.addAll(query.getResultList());
			res = 0;
		} catch (Exception e) {
			logger.error("PaymentMethodService.complete:" + e.getMessage(), e);
		}
		return res;
	}
