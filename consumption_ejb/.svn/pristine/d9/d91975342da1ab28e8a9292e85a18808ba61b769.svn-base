...
public class CustomerPricingProgramService implements ICustomerPricingProgramService {
	...
	public int caphnhatlaingay(List<Long> ids, Date sDate, Date eDate) {
		...
		try {
			if (ids != null && ids.size() != 0) {
				Query query = em
						.createQuery("update CustomerPricingProgram set effective_date=:sDate, expiry_date=:eDate where id in :ids");
	...
	@Override
	public int insertOrUpdate(CustomerPricingProgramReqInfo t) {
	    int res = -1;
	    try {
	        CustomerPricingProgram p = t.getCustomer_pricing_program();
	        if (p != null) {
	            Query query = em.createQuery(
	                "UPDATE CustomerPricingProgram p SET " +
	                "p.effective_date = :eff, " +
	                "p.expiry_date = :exp, " +
	                "p.disable = :dis, " +
	                "p.note = :note " +
	                "WHERE p.pricing_program = :pc AND p.customer = :c"
	            );
	            query.setParameter("eff", p.getEffective_date());
	            query.setParameter("exp", p.getExpiry_date());
	            query.setParameter("dis", p.isDisable());
	            query.setParameter("note", p.getNote());
	            query.setParameter("pc", p.getPricing_program());
	            query.setParameter("c", p.getCustomer());
	            ...
	            if (rowsUpdated == 0) {
	                em.persist(p); // Không có -> insert mới
	            }
	    	...
	    	e.printStackTrace();
	        logger.error("CustomerPricingProgramService.insertOrUpdate: " + e.getMessage(), e);
	    }
	    return res;
	}
	...
	public int selectForCustomer(String json, CustomerPricingProgramReqInfo t) {
		...
		try {
			...
			if (hDeliveryDate.getErr() == 0 && hCustomerId.getErr() == 0) {
				...
				dis3.getExpressions().add(
						cb.greaterThanOrEqualTo(root.get("pricing_program").get("expiry_date"), pDeliveryDate));
				predicates.add(dis3);
