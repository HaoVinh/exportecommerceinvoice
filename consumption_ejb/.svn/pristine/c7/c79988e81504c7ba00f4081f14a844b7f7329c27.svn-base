...
public class PromotionProgramDetailService implements IPromotionProgramDetailService {
	...
	public List<PromotionProgramDetail> findAllByPromotionProgram(long idprograms) {
		try {
			...
			logger.error("PromotionProgramDetailService.findAllByPricingProgram:" + e.getMessage(), e);
		}
		return new ArrayList<PromotionProgramDetail>();
	}
	...
	public int search(String json, PagingInfo page, List<PromotionProgramDetail> list) {
		...
		try {/*
			...
			HolderParser hProgramId = JsonParserUtil.getValueNumber(j.get("promotion_program_detail_info"),
					"program_id", null);
			HolderParser hProductId = JsonParserUtil.getValueNumber(j.get("promotion_program_detail_info"),
					"product_id", null);
			HolderParser hPromotionProductId = JsonParserUtil.getValueNumber(j.get("promotion_program_detail_info"),
			...
			Join<PromotionProgramDetail, Product> productPromotion_ = (Join) root_.fetch("promotion_product",
					JoinType.INNER);
			...
			cq.select(root_)
					...
					.orderBy(cb.asc(product_.get("product_code")), cb.asc(root_.get("promotion_form")),
							cb.asc(productPromotion_.get("product_code")));
			TypedQuery<PromotionProgramDetail> query = em.createQuery(cq);
			...
			query.setParameter(pPromotionProductId,
					Long.parseLong(Objects.toString(hPromotionProductId.getValue(), "0")));
			...
			query2.setParameter(pPromotionProductId,
					Long.parseLong(Objects.toString(hPromotionProductId.getValue(), "0")));
	...
	public int insert(PromotionProgramDetailReqInfo t) {
		...
		try {
			...
			if (p != null) {
				...
				// int check=checkExist(product !=null ? product.getId() :0,
				// promotionProduct !=null ? promotionProduct.getId()
				// :0,p.getId(),
				// p.getPromotion_program().getId(),p.getPromotion_form());
				// res=check;
				// if(check==0){
				em.persist(p);
				if (p.getId() > 0) {
	...
	public int update(PromotionProgramDetailReqInfo t) {
		...
		try {
			...
			if (p != null) {
				Product product = p.getProduct();
				Product promotionProduct = p.getPromotion_product();
				// (long productId,long promotionProductId,long id,long
				// programId,int form)
				// int check=checkExist(product !=null ? product.getId() :0,
				// promotionProduct !=null ? promotionProduct.getId()
				// :0,p.getId(),
	...
	@Override
	public int updateByPredicate(long productId, long productProId, long promotion_form, long programId,
			double box_quatity, double promotion_quantity) {
		int res = -1;
		try {
			StringBuilder sql = new StringBuilder();
			sql.append("update PromotionProgramDetail set box_quatity=:q, promotion_quantity=:qkm where product_id=:pid and promotion_product_id=:pdid and promotion_form=:htkm promotion_program.id =:ppi ");
			Query query = em.createQuery(sql.toString());
			query.setParameter("q", box_quatity);
			query.setParameter("qkm", promotion_quantity);
			query.setParameter("pid", productId);
			query.setParameter("pdid", productProId);
			query.setParameter("htkm", promotion_form);
			query.setParameter("ppi", programId);
			return query.executeUpdate();
		} catch (Exception e) {
			logger.error("PromotionProgramDetailService.updateByPredicate:" + e.getMessage(), e);
		}
		return res;
	}
	...
	public int selectBy(String json, List<PromotionProgramDetail> list) {
		int res = -1;
		/*
		 * {product_id:0,promotion_product_id:0,promotion_program_id:0,
		 * promotion_form:-1}
		 */
		try {
			JsonObject j = JsonParserUtil.getGson().fromJson(json, JsonObject.class);
			...
			dis3.getExpressions().add(cb.equal(pPromotionForm, -1));
