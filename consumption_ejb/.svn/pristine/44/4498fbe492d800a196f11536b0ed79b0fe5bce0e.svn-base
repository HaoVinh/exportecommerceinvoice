...
public class StockerService implements IStockerService {
	...
	public int insert(StockerReqInfo t) {
		...
		try {
			...
			if (p != null) {
				if (p.getStocker_code() == null || "".equals(p.getStocker_code())) {
	...
	private String initCode() {
		try {
			...
			cq.multiselect(cb.coalesce(cb.max(cb.quot((Expression) root.get("stocker_code"), 1)), 0));
	...
	@Override
	public Stocker selectByCode(String code) {
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<Stocker> cq = cb.createQuery(Stocker.class);
			Root<Stocker> root = cq.from(Stocker.class);
	...
	@Override
	public Stocker selectByName(String name) {
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<Stocker> cq = cb.createQuery(Stocker.class);
			Root<Stocker> root = cq.from(Stocker.class);
			cq.select(root).where(cb.equal(root.get("stocker_name"), name));
			TypedQuery<Stocker> query = em.createQuery(cq);
			List<Stocker> stockers= query.getResultList();
			if(stockers.size()!=0)
				return stockers.get(0);
		} catch (Exception e) {
		}
		return null;
	}
	...
	@Override
	public int complete(String text, List<Stocker> list) {
		int res = -1;
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<Stocker> cq = cb.createQuery(Stocker.class);
			Root<Stocker> root = cq.from(Stocker.class);
			Predicate dis = cb.disjunction();
			dis.getExpressions().add(cb.equal(root.get("stocker_code"), text));
			dis.getExpressions().add(cb.like(root.get("stocker_name"), "%" + text + "%"));
			cq.select(root).where(dis,cb.equal(root.get("disable"),false));
			TypedQuery<Stocker> query = em.createQuery(cq);
			list.addAll(query.getResultList());
			res = 0;
		} catch (Exception e) {
