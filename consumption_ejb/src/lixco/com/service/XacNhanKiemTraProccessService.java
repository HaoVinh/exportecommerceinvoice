...
public class XacNhanKiemTraProccessService {
	...
	public List<XacNhanKiemTraDTO> searchAPI(Date startDate, Date endDate) {
		try {
			...
			cq.select(cb.construct(XacNhanKiemTraDTO.class, root_.get("id"), root_.get("createdBy"),
					root_.get("requestCode"), root_.get("requestDate"), root_.get("trangthaicl"), root_.get("note"),
	...
	public List<XacNhanKiemTraDTO> searchAPIKhongDat(Date startDate, Date endDate) {
		try {
			...
			for (int i = 0; i < sizeYCs; i++) {
				List<XacNhanKiemTraDetailDTO> xacNhanKiemTraDetailDTOs = xacNhanKiemTraDetailService
						.findChiTietKhongDatByPhieuId(xacNhanKiemTraDTOs.get(i).getId());
				xacNhanKiemTraDTOs.get(i).setXacNhanKiemTraDetailDTOs(xacNhanKiemTraDetailDTOs);
	...
	public int updateKetQuaKiemTra(XacNhanKiemTraDTO pDTO, StringBuilder messages)
	...
	public int saveOrUpdate(XacNhanKiemTra xacNhanKiemTra, Message messages)
			throws IllegalStateException, SystemException, SQLException {
		int res = -1;
		Connection con = null;
		try {
