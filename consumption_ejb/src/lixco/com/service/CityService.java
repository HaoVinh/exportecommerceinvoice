...
public class CityService implements ICityService {
	...
	public int findLike(String text, int size, List<City> list) {
		...
		try {
			...
			dis.getExpressions().add(
					cb.like(cb.function("replace", String.class, cb.lower(root.get("city_name")), cb.literal("đ"),
	...
	public int complete(String text, List<City> list) {
		...
		try {
			...
			dis.getExpressions().add(cb.like(root.get("city_name"), "%" + text + "%"));
			cq.select(
					cb.construct(City.class, root.get("id"), root.get("city_code"), root.get("city_name"),
							root.get("en_name"))).where(dis);
	...
	public int complete(String text, long areaId, List<City> list) {
		...
		try {
			...
			if (areaId != 0) {
				con.getExpressions().add(cb.equal(root.get("area").get("id"), areaId));
			}
			Predicate dis = cb.disjunction();
			dis.getExpressions().add(cb.equal(root.get("city_code"), text));
			dis.getExpressions().add(cb.like(root.get("city_name"), "%" + text + "%"));
