...
public class IECategoriesService implements IIECategoriesService {
	...
	@Override@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<IECategories> findAll() {
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<IECategories> cq = cb.createQuery(IECategories.class);
			Root<IECategories> root = cq.from(IECategories.class);
		...
		return new ArrayList<IECategories>();
	}
	...
	public int selectByCodeOld(String code, IECategoriesReqInfo t) {
		...
		try {
			...
			TypedQuery<IECategories> query = em.createQuery(cq);
			List<IECategories> results = query.setMaxResults(1).getResultList();
			if (results.size() != 0)
				t.setIe_categories(results.get(0));
			res = 0;
		} catch (Exception e) {
	...
	public String initCode() {
		try {
			...
			cq.multiselect(cb.coalesce(cb.max(cb.quot((Expression) cb.substring(root.get("code"), 3), 1)), 0));
	...
	@Override@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public IECategories selectByCodeOld(String code) {
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<IECategories> cq = cb.createQuery(IECategories.class);
			Root<IECategories> root = cq.from(IECategories.class);
			Predicate dis = cb.disjunction();
			dis.getExpressions().add(cb.equal(root.get("code"), code));
			cq.select(cb.construct(IECategories.class, root.get("id"), root.get("code"), root.get("content"))).where(
					dis);
			TypedQuery<IECategories> query = em.createQuery(cq);
	...
	@Override@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public IECategories selectByCode(String code) {
		try {
			Query query = em.createQuery("select new IECategories(id)  from IECategories where code=:cd");
			query.setParameter("cd", code);
			return (IECategories) query.getSingleResult();
		} catch (Exception e) {
		}
		return null;
	}
	...
	@Override@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public IECategories selectByCodeAll(String code) {
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<IECategories> cq = cb.createQuery(IECategories.class);
			Root<IECategories> root = cq.from(IECategories.class);
	...
	@Override@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public int complete(String text, List<IECategories> list) {
		int res = -1;
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<IECategories> cq = cb.createQuery(IECategories.class);
			Root<IECategories> root = cq.from(IECategories.class);
			Predicate dis = cb.disjunction();
			dis.getExpressions().add(cb.equal(root.get("code"), text));
			dis.getExpressions().add(cb.like(root.get("content"), "%" + text + "%"));
			cq.select(cb.construct(IECategories.class, root.get("id"), root.get("code"), root.get("content"))).where(
					dis,cb.equal(root.get("disable"), false));
			TypedQuery<IECategories> query = em.createQuery(cq);
			list.addAll(query.getResultList());
			res = 0;
		} catch (Exception e) {
	...
	@Override@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public int completeCode(String text, List<IECategories> list) {
		int res = -1;
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<IECategories> cq = cb.createQuery(IECategories.class);
			Root<IECategories> root = cq.from(IECategories.class);
			if ("".equals(text)) {
				cq.select(cb.construct(IECategories.class, root.get("id"), root.get("code"), root.get("content"))).where(cb.equal(root.get("disable"), false));
			} else {
				cq.select(cb.construct(IECategories.class, root.get("id"), root.get("code"), root.get("content")))
						.where(cb.equal(root.get("code"), text),cb.equal(root.get("disable"), false));
			}
			TypedQuery<IECategories> query = em.createQuery(cq);
			list.addAll(query.getResultList());
			res = 0;
