...
public class XacNhanKiemTraServlet {
	...
	@GET
	@Path("danhsachphieukhongdat")
	@Produces("application/json; charset=utf-8")
	public Response findDanhSachPhieuKD(
	    @QueryParam(DefinedName.PARAM_SDATE) String sDate,
	    @QueryParam(DefinedName.PARAM_EDATE) String eDate,
	    @QueryParam("trangthaicl") String trangthaiclStr) throws IOException, ServletException {
	    ...
	    try {
	        ...
	        Date endDate = MyUtil.chuyensangStrApiDate(eDate);
//	        int trangthaicl = 0;
//	        if (trangthaiclStr != null && !trangthaiclStr.trim().isEmpty()) {
//	            try {
//	                trangthaicl = Integer.parseInt(trangthaiclStr.trim());
//	            } catch (NumberFormatException e) {
//	                result.append("trangthaicl không hợp lệ");
//	                return responseBuilder.entity(result.toString()).build();
//	            }
//	        }

	        List<XacNhanKiemTraDTO> xacNhanKiemTraDTOs = 
	            xacNhanKiemTraProccessService.searchAPIKhongDat(startDate, endDate);

	        if (xacNhanKiemTraDTOs != null && !xacNhanKiemTraDTOs.isEmpty()) {
	            JsonObject jsonObject = new JsonObject();
	            jsonObject.add("data", JsonParserUtil.getGson().toJsonTree(xacNhanKiemTraDTOs));
	            responseBuilder.status(Response.Status.OK);
	            result.append(jsonObject);
