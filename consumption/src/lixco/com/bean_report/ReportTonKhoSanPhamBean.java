...
import lixco.com.report_service.TonKhoSanPhamTheoNgay;
...
public class ReportTonKhoSanPhamBean extends AbstractBean {
	...
	public void proccessTonKho() {
		...
		// TonKhoDauThang tk =
		// reportService.reportTonKhoDauThang(lc.getMonthOfYear(), lc.getYear(),
				...
				for (int i = 0; i < listTonKhoSanPham.size(); i++) {
					Object[] row = {
							...
							listTonKhoSanPham.get(i).getPromotion_product_name() == null ? "" : listTonKhoSanPham
									.get(i).getPromotion_product_name(), listTonKhoSanPham.get(i).getNhanhang() };
					results.add(row);
	...
	public void reportTonKhoSanPhamExcel_ChiTietTheoNgay() {
		Notify notify = new Notify(FacesContext.getCurrentInstance());
		try {
			JsonObject json = new JsonObject();
			json.addProperty("from_date", ToolTimeCustomer.convertDateToString(fromDateSearch, "dd/MM/yyyy"));
			...
			List<TonKhoSanPhamTheoNgay> listTonKhoSanPham = new ArrayList<TonKhoSanPhamTheoNgay>();
			reportService.reportTonKhoSanPhamChiTietTheoNgay(JsonParserUtil.getGson().toJson(json), listTonKhoSanPham);
			
			if (listTonKhoSanPham.size() > 0) {
				List<Object[]> results = new ArrayList<Object[]>();
				Object[] title = { "NGÀY", "MÃ LOẠI SP", "TÊN LOẠI SP", "MÃ SP BRAND", "TÊN SP BRAND", "LEVER CODE", "MASP",
						"SẢN PHẨM", "TỒN ĐẦU(Kg)", "TỒN ĐẦU(ĐVT)", "NHẬP(Kg)", "NHẬP(ĐVT)", "SL.XUẤT(Kg)",
						"SL.XUẤT(ĐVT)", "SL.CUỐI(Kg)", "SL.CUỐI(ĐVT)", "HSQD", "SL CARTON", "MÃ SPKM", "TÊN SPKM",
						"NHÃN HÀNG" };
				results.add(title);
				...
				for (int i = 0; i < listTonKhoSanPham.size(); i++) {
					TonKhoSanPhamTheoNgay item = listTonKhoSanPham.get(i);
					item.setFactor(item.getFactor()==0?1:item.getFactor());
					Object[] row = {
							ToolTimeCustomer.convertDateToString(item.getTransaction_date(), "dd/MM/yyyy"),
							...
							item.getKg_opening_balance(),
							MyMath.roundCustom(item.getKg_opening_balance()/item.getFactor(), 2),
							
							MyMath.roundCustom(item.getKg_import_quantity(),2),
							MyMath.roundCustom(item.getUnit_import_quantity(), 2),
							
							MyMath.roundCustom(item.getKg_export_quantity(),2),
							MyMath.roundCustom(item.getUnit_export_quantity(), 2),
							
							item.getKg_closing_balance(),
							
							MyMath.roundCustom(item.getKg_closing_balance()
									/ item.getFactor(), 2),
							item.getFactor(),
							item.getSpecification(),
							item.getPromotion_product_code() == null ? "" : item.getPromotion_product_code(),
							item.getPromotion_product_name() == null ? "" : item.getPromotion_product_name(),
							item.getNhanhang() == null ? "" : item.getNhanhang()
					};
					results.add(row);
				}
				
				StringBuilder title2 = new StringBuilder();
				title2.append("tonsp_theo_ngay_ ");
				if (fromDateSearch != null)
			...
			logger.error("ReportTonKhoSanPhamBean.reportTonKhoSanPhamExcel_ChiTietTheoNgay:" + e.getMessage(), e);
		}
	}
