	...
	public void exportExcel() {
		try {
			if (listProductType.size() > 0) {
				...
				Object[] title = { "id", "maloai", "tenloai", "tenloai(en)" };
				results.add(title);
				for (ProductType p : listProductType) {
					Object row[] = { p.getId(), p.getCode(),p.getName(),p.getEn_name()};
					results.add(row);
				}
				String titleEx = "dmloaisp";
				...
				while (rows.hasNext()) {
					...
					lv2: while (cells.hasNext()) {
						...
						case 0:
							try {
								// mã loại sản phẩm
								String malsp = Objects.toString(MyUtilExcel.getCellValue(cell), null);
						...
						case 1:
							try {
								// tên loại sản phẩm
								String name_type = Objects.toString(MyUtilExcel.getCellValue(cell), null);
						...
						case 2:
							try {
								...
								String name_ = Objects.toString(MyUtilExcel.getCellValue(cell), null);
