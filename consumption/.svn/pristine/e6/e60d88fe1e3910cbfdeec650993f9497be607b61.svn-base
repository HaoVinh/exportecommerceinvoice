...
public class KeHoachGiaoHangBean extends AbstractBean {
	...
	@Getter
	@Setter
	boolean lay_hcm = true;
	@Getter
	@Setter
	boolean lay_bd = true;

	public void capnhatdulieu() {
		keHoachGiaoHangs = orderDetailService.findBy(lay_hcm, lay_bd, ngaygh);
//		for (int i = 0; i < keHoachGiaoHangs.size(); i++) {
//				keHoachGiaoHangs.get(i).setKhoiluongvc(keHoachGiaoHangs.get(i).getKhoiluongvctinh());
//		}
		if (bd_hcm) {
			String chinhan_process = null;
			...
			if (chinhan_process != null) {
				List<KeHoachGiaoHang> khghs = KeHoachGiaoHangProccess.dasxtheonhomhang(lay_hcm, lay_bd,chinhan_process,
						accountAPIService, ngaygh);
				if (!khghs.isEmpty()) {
					for (int i = 0; i < khghs.size(); i++) {
						String makh = khghs.get(i).getCustomer() != null ? khghs.get(i).getCustomer()
	...
	public int tongXe(String kenh) {
		return keHoachGiaoHangs.stream().filter(i -> kenh.equals(i.getKenhkhachhang()))
				.mapToInt(KeHoachGiaoHang::getSlxe).sum();
	}

	public double tongKhoiLuong(String kenh) {
		return keHoachGiaoHangs.stream().filter(i -> kenh.equals(i.getKenhkhachhang()))
				.mapToDouble(KeHoachGiaoHang::getKhoiluongvc).sum();
	}
