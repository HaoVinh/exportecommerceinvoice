...
public class KeHoachGiaoHangServlet {
	...
	public Response dasxtheonhomhang(@QueryParam(DefinedName.PARAM_NDATE) String date) throws IOException,
			ServletException {
		StringBuilder result = new StringBuilder();
		ResponseBuilder responseBuilder = Response.status(Response.Status.BAD_REQUEST);
		try {
			Date ngay = MyUtil.chuyensangStrApiDate(date);
			List<KeHoachGiaoHang> dasxtheonhomhang = orderDetailService.findBy2(true, true, ngay);
			if (dasxtheonhomhang.size() != 0) {
				for (int i = 0; i < dasxtheonhomhang.size(); i++) {
					if (dasxtheonhomhang.get(i).getKhoiluongvc() == 0) {
						dasxtheonhomhang.get(i).setKhoiluongvc(
								orderDetailService.khoiluongtongdonhang(dasxtheonhomhang.get(i).getIdorder()));
					}
				}
