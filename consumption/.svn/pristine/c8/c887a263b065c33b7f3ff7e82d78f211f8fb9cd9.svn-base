...
public class ProductGroupBean extends AbstractBean  {
	...
	public void exportExcel() {
		try {
			if (listProductGroup.size() > 0) {
				List<Object[]> results = new ArrayList<Object[]>();
				Object[] title = { "id", "manhom", "tennhom" };
				results.add(title);
				for (ProductGroup p : listProductGroup) {
					Object row[] = { p.getId(), p.getCode(),p.getName()};
					results.add(row);
				}
				String titleEx = "dmnhomsp";
				...
				while (rows.hasNext()) {
					...
					lv2: while (cells.hasNext()) {
						...
						case 0:
							try {
								// mã nhóm sản phẩm
								String mansp = Objects.toString(MyUtilExcel.getCellValue(cell), null);
