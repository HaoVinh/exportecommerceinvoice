...
public class OrderLixBean extends AbstractBean {
	...
	boolean donhangmiennam = false;
	final String[] dmshoadonphiabac = { "CL056", "CL449", "CS169", "CL317", "CL326", "CL405", "DN089", "CO299",
			"CL523", "NCT75" };

	public void taidonhangDMS_MienNam() {
		try {
			if (psDate == null) {
				noticeError("Chưa nhập ngày để lọc ");
			} else {
				...
				donHangNPPAPIs = new ArrayList<DonHangNPPAPI>();
				donHangNPPAPIFilters = new ArrayList<DonHangNPPAPI>();
				String data = CallAPIMBNapVaoMN.dsdonHangDMS("tkphiabac", psDate, peDate);
				GsonBuilder gsonBuilder = new GsonBuilder().setPrettyPrinting();
				gsonBuilder.registerTypeAdapter(Date.class, new DateJsonSerializer());
				gsonBuilder.registerTypeAdapter(Date.class, new DateJsonDeserializer());
				Gson gson = gsonBuilder.create();
				CallAPIData callAPIData = gson.fromJson(data, CallAPIData.class);
				if (callAPIData != null && "0".equals(callAPIData.getErr())) {
					if (callAPIData.getDt() != null) {
						final DonHangNPPAPI[] dsarray = gson.fromJson(callAPIData.getDt(), DonHangNPPAPI[].class);
						for (int i = 0; i < dsarray.length; i++) {
							for (int j = 0; j < dmshoadonphiabac.length; j++) {
								if (dsarray[i].getMakhachhang().equals(dmshoadonphiabac[j])) {
									donHangNPPAPIs.add(dsarray[i]);
									break;
								}
							}
						}
				...
				if (donHangNPPAPIs.get(i).isChonphieu()) {
					...
					if (status) {
						...
						if (customer != null) {
							...
							orderLix.setTax_value(GiaTriMacDinh.HESOTHUE);
							String maxn = "";
							...
							if ("BN341".equals(makh)) {
								if ("X".equals(maxn)) {
									maxn = "E";
								} else if ("A".equals(maxn)) {
									maxn = "5";
								}
							}

							if (customer.getMaxn() != null && !"".equals(customer.getMaxn().trim())) {
								maxn = customer.getMaxn();
							}
							...
							if (chk == 0) {
								// Xac nhan don hang
								if (donhangmiennam) {
									xacnhandonhangDMS_MienNam(orderLix.getNpp_order_id(), orderLix.getVoucher_code());
								} else {
											...
											if (orderLix.getPromotion_program() != null) {
												if (orderDetail.getPromotion_forms() != 0) {
													...
													for (PromotionProgramDetail ppd : programDetails) {
														...
														double quantity = (double) ((int) (orderDetail
																.getBox_quantity() / ppd.getBox_quatity()))
	...
	public void boxacnhanDMS(DonHangNPPAPI donHangNPPAPI) {
		...
		try {
			...
			String data = "";
			if (donhangmiennam) {
				data = CallAPIMBNapVaoMN.boxacnhandonHangDMS(donHangNPPAPI.getIddonhang());
			} else {
				data = CallAPI.boxacnhandonHangDMS(donHangNPPAPI.getIddonhang());
			}
			GsonBuilder gsonBuilder = new GsonBuilder().setPrettyPrinting();
			gsonBuilder.registerTypeAdapter(Date.class, new DateJsonSerializer());
			gsonBuilder.registerTypeAdapter(Date.class, new DateJsonDeserializer());
											...
											if (orderDetail.getPromotion_forms() != 0) {
												...
												if (programDetails.size() != 0) {
													for (PromotionProgramDetail ppd : programDetails) {
														...
														// CriteriaQuery<PromotionalPricing>
														// cqPromotionPricing =
														// cb.createQuery(PromotionalPricing.class);
														// Root<PromotionalPricing>
														// rootPromotionPricing_
														// = cqPromotionPricing
														// .from(PromotionalPricing.class);
														// Predicate cons =
														// cb.conjunction();
														// cons.getExpressions().add(
														// cb.lessThanOrEqualTo(rootPromotionPricing_.get("effective_date"),
														// deliveryDate));
														// cons.getExpressions().add(
														// cb.greaterThanOrEqualTo(rootPromotionPricing_.get("expiry_date"),
														// deliveryDate));
														// cons.getExpressions().add(
														// cb.equal(rootPromotionPricing_.get("product").get("id"),
														// ppd.getPromotion_product()
														// .getId()));
														// cqPromotionPricing.select(rootPromotionPricing_).where(cons)
														// .orderBy(cb.desc(rootPromotionPricing_.get("effective_date")));
														// TypedQuery<PromotionalPricing>
														// queryPromotionPricing
														// =
														// em.createQuery(cqPromotionPricing);
														// List<PromotionalPricing>
														// listPromotionPricingTrans
														// =
														// queryPromotionPricing.getResultList();
														// if
														// (listPromotionPricingTrans.size()
														// > 0) {
														// pod.setUnit_price(listPromotionPricingTrans.get(0).getUnit_price());
														// }
														// // int quantity =
														// (int)
														// (orderDetail.getBox_quantity()
														// /
														// ppd.getBox_quatity())
														// // * (int)
														// ppd.getSpecification();
														...
														// // int quantityAct =
														// (int)
														// (orderDetail.getBox_quantity_actual()
														// /
														// ppd.getBox_quatity())
														// // * (int)
														// ppd.getSpecification();
					...
					lv1: while (rows.hasNext()) {
						...
						while (cells.hasNext()) {
							...
							case 0:
								try {
									// masp
									String masp = Objects.toString(dataFormatter.formatCellValue(cell), null);
									// try {
									// double maspIsNumber =
									// Double.parseDouble(masp);
	...
	public void xacnhandonhangDMS_MienNam(long iddonhang, String madonhang) {
		try {
			String data = CallAPIMBNapVaoMN.xacnhandonHangDMS(iddonhang);
	...
	protected void initItem() {
		try {
			...
			System.out.println("Thời gian thực thi tai don hang: " + duration + " mili giây");
	...
	public void capnhatKLVC() {
		try {
			// kiểm tra hóa đơn có tồn tại không
			double khoiluongvc = 0;
			for (int i = 0; i < listOrderDetail.size(); i++) {
				double soconlai = listOrderDetail.get(i).getBox_quantity()
						- listOrderDetail.get(i).getBox_quantity_actual();
				if (soconlai > 0) {
					double kl = soconlai * listOrderDetail.get(i).getProduct().getSpecification()
							* listOrderDetail.get(i).getProduct().getFactor();
					khoiluongvc += kl;
				}
			}
			for (int i = 0; i < listPromotionOrderDetail.size(); i++) {
				double soconlai = listPromotionOrderDetail.get(i).getQuantity()
						- listPromotionOrderDetail.get(i).getQuantityAct();
				if (soconlai > 0) {
					double kl = soconlai * listPromotionOrderDetail.get(i).getProduct().getFactor();
					khoiluongvc += kl;
				}
			}
			orderLixCrud.setKhoiluongvc(Math.round(khoiluongvc));
		} catch (Exception e) {
			logger.error("OrderLixBean.tabInvoiceOnClick:" + e.getMessage(), e);
		}
	}
				...
				if (obj != null) {
					...
					if (orderLixCrud.getIe_categories() == null) {
						...
						if (cus != null) {
							...
							if (cus.getMaxn() != null && !"".equals(cus.getMaxn().trim())) {
								maxnIns = cus.getMaxn();
							}
							if (maxnIns != null && !"".equals(maxnIns.trim())) {
								orderLixCrud.setIe_categories(ieCategoriesService.selectByCode(maxnIns));
							}
						}
					}
				} else {
	...
	@Getter
	@Setter
	private Date delivery_dateSearch;
	@Getter
	@Setter
	private Date ngaygioxevaoSearch;
	...
	public void search() {
		try {
			...
			jsonInfo.addProperty("delivery_date", ToolTimeCustomer.convertDateToString(delivery_dateSearch, "dd/MM/yyyy"));
			jsonInfo.addProperty("ngaygioxevao", ToolTimeCustomer.convertDateToString(ngaygioxevaoSearch, "dd/MM/yyyy"));
	...
	public void loadOrderLixDetail() {
		try {
			...
			if (listOrderDetail.size() != 0) {
				...
				// for (int i = 0; i < listOrderDetail.size(); i++) {
				// System.out.println(listOrderDetail.get(i).getColumnsort()+" - "+listOrderDetail.get(i).getIndexsort()+" - "+listOrderDetail.get(i).getId()+" - "+listOrderDetail.get(i).getProduct().getProduct_code());
				// }
				if (dataTable != null && listOrderDetail.size() != 0) {
					if (columnSort != null) {
						dataTable
								.setValueExpression("sortBy", createValueExpression("#{item.indexsort}", Object.class));
				...
				if (allowUpdate(null)) {
					...
					if (listData.size() > 0) {
						...
						int code = processLogicInvoiceService.createInvoiceByExtensionOrder(data, listIdResult,
								messages, null, ht.isKiemton(), null);
						if (code == 0) {
		...
		try {
			if (orderLixCrud != null && orderLixCrud.getId() != 0) {
				if (allowUpdate(null)) {
					...
					if (listOrderLix != null) {
						int index = listOrderLix.indexOf(orderLixReqInfo.getOrder_lix());
						if (index != -1) {
							listOrderLix.set(index, orderLixReqInfo.getOrder_lix());
						}
					}
			...
			e.printStackTrace();
			noticeError(messages.toString() + e.getLocalizedMessage());
		}
	}
					...
					if (t.getExport_quantity() > 0) {
						if (orderLixCrud.getPromotion_program() != null) {
							...
							lv1: for (PromotionProgramDetail ppd : list) {
								...
								double quantity = (double) ((int) t.getExport_quantity() / ppd.getBox_quatity() * ppd
										.getSpecification());
								pod.setQuantity(quantity);
	...
	public void onSort(SortEvent event) {
		...
		// listOrderDetail = (List<OrderDetail>) dataTable.getValue();
			...
			if ("indonhangtonghop".equals(loaibaocao)) {
				...
				if (orderChooses.size() != 0) {
					...
					for (int i = 0; i < objspchinhs.size(); i++) {
						...
						sp.setStt(i + 1);
						sp.setTensanpham(objspchinhs.get(i)[4] + " - " + objspchinhs.get(i)[0]);
					...
					for (int i = 0; i < objspkhuyenmais.size(); i++) {
						SanPhamDonHang sp = new SanPhamDonHang();
						sp.setStt(i + 1);
						sp.setTensanpham(objspkhuyenmais.get(i)[4] + " - " + objspkhuyenmais.get(i)[0]);
	...
	public void exportPDF() {
		...
		try {
			if (orderLixCrud != null && orderLixCrud.getId() > 0) {
				...
				for (int i = 0; i < listOrderDetail.size(); i++) {
					if (listOrderDetail.get(i).isHuyct()) {
						...
						trongtai += listOrderDetail.get(i).getBox_quantity()
								...
								* listOrderDetail.get(i).getProduct().getFactor();
						trongtaibb += listOrderDetail.get(i).getBox_quantity()
								* listOrderDetail.get(i).getProduct().getTare();
					}
				...
				importParam.put("hst", orderLixCrud.getTax_value());

				listPromotionOrderDetail.removeIf(PromotionOrderDetail::isHuyct);

				LinkedHashMap<Long, Double> totalsById = new LinkedHashMap<>();
				LinkedHashMap<Long, Product> firstProductById = new LinkedHashMap<>();

				for (PromotionOrderDetail pod : listPromotionOrderDetail) {
					Product p = pod.getProduct();
					long pid = p.getId(); // giả sử có getId()

					firstProductById.putIfAbsent(pid, p);
					totalsById.merge(pid, pod.getQuantity(), Double::sum);
				}

				List<PromotionOrderDetail> listPromoParams = new ArrayList<>();
				int idx = 1;
				for (Map.Entry<Long, Double> e : totalsById.entrySet()) {
					long pid = e.getKey();
					double quantityTotal = e.getValue();
					Product key = firstProductById.get(pid);
					...
					trongtai += quantityTotal * key.getFactor();
					trongtaibb += (quantityTotal / key.getSpecification()) * key.getTare();
				}

				importParam.put("trongtai", (double) MyMath.round(trongtai));
				importParam.put("trongtaibb", (double) MyMath.round(trongtaibb));
	...
	public void napmalohang(Workbook workBook) {
		Notify notify = new Notify(FacesContext.getCurrentInstance());
		PrimeFaces current = PrimeFaces.current();
		try {
			if (allowUpdate(null)) {
				...
				lv1: while (rows.hasNext()) {
					...
					while (cells.hasNext()) {
						...
						case 1:
							try {
								// masp
								String masp = Objects.toString(MyUtilExcel.getCellValue(cell, true), null);
				...
				for (int i = 0; i < listOrderDetail.size(); i++) {
					for (OrderDetail it : listDetail) {
						if (listOrderDetail.get(i).getProduct().getProduct_code()
								.equals(it.getProduct().getProduct_code())) {
							listOrderDetail.get(i).setBatch_code(it.getBatch_code());
							orderDetailService.updateMaLH(listOrderDetail.get(i));
						}
					}
				}
				// show thông tin chi tiết đơn hàng
			...
			if (allowUpdate(null)) {
				if (event.getFile() != null) {
					...
					if (napMaLH) {
						...
						lv1: while (rows.hasNext()) {
							...
							while (cells.hasNext()) {
											...
											if (product != null) {
												...
												if (orderLixCrud.getPricing_program() != null) {
													...
													long idPriceSub = customerPricingProgramService
															.selectForCustomerSub(orderLixCrud.getPricing_program()
																	.getId(), orderLixCrud.getOrder_date(), lix
																	.getProduct().getId());
													if (idPriceSub != 0) {
													...
													if (statusPrice) {
														pricingProgramDetailService.findSettingPricing(orderLixCrud
																.getPricing_program().getId(),
																lix.getProduct().getId(), t1);
														if (t1.getPricing_program_detail() != null) {
															if (t1.getPricing_program_detail().getUnit_price() != 0) {
																lix.setUnit_price(t1.getPricing_program_detail()
																		.getUnit_price());
												...
												if (orderLixCrud.getIe_categories() != null)
													for (int i = 0; i < maspcaidgkm.length; i++) {
														if (maspcaidgkm[i].equals(orderLixCrud.getIe_categories()
																.getCode()) && orderLixCrud.getTax_value() == 0) {
															caidgkm = true;
															break;
														}
													}
												if (caidgkm) {
													...
													if (ppr.getPromotional_pricing() != null) {
														...
														lix.setUnit_price_goc(ppr.getPromotional_pricing()
																.getUnit_price());
													}
												}
											}
								...
								case 5:
									...
									lix.setBatch_code(malohang);
									break;
								case 6:// don gia PO cua khach hang
									try {
										String dgPOStr = Objects.toString(MyUtilExcel.getCellValue(cell), "0");
										double dgPO = Double.parseDouble(dgPOStr);
										lix.setUnit_pricePO(dgPO);
									} catch (Exception e) {
									}
									break;
								}
							...
							if (orderDetailService.insert(p) == 0) {
								...
								if (it.getPromotion_forms() != 0 && orderLixCrud.getPromotion_program() != null) {
									...
									for (PromotionProgramDetail ppd : list) {
										...
										/*
										 * Cài đặt ngày 14/05/2024 chị Thu Ha
										 * Nếu KM lấy giá bán cài như sau
										 */
										if (kmlaygiaban) {
											if (orderLixCrud.getPricing_program() != null) {
												boolean statusPrice = true;
												...
												long idPriceSub = customerPricingProgramService.selectForCustomerSub(
														...
														orderLixCrud.getOrder_date(), pod.getProduct().getId());
												if (idPriceSub != 0) {
													pricingProgramDetailService.findSettingPricing(idPriceSub, pod
															.getProduct().getId(), t1);
													if (t1.getPricing_program_detail() != null) {
														if (t1.getPricing_program_detail().getUnit_price() != 0) {
															pod.setUnit_price(t1.getPricing_program_detail()
																	.getUnit_price());
															statusPrice = false;
														}
													}
										...
										// double quantity = BigDecimal
										// .valueOf(it.getBox_quantity())
										// .multiply(BigDecimal.valueOf(ppd.getSpecification()))
										// .divide(BigDecimal.valueOf(ppd.getBox_quatity()),
										// MathContext.DECIMAL32)
										// .doubleValue();

										double quantity = (double) ((int) (it.getBox_quantity() / ppd.getBox_quatity()))
												* ppd.getSpecification();
										pod.setQuantity(quantity);
										pod.setCreated_date(new Date());
										pod.setCreated_by(account.getMember().getName());
						...
						listOrderDetail = new ArrayList<>();
						orderDetailService.selectByOrder(orderLixCrud.getId(), listOrderDetail);
						// show thông tin sản phẩm khuyến khuyến mãi của đơn
						// hàng
						listPromotionOrderDetail = new ArrayList<>();
						promotionOrderDetailService.selectByOrder(orderLixCrud.getId(), listPromotionOrderDetail);
						sumOrderLix();
			...
			if (orderChooses.size() != 0) {
				...
				if (orderDetails.size() > 0) {
					...
					Object[] title = { "ID_ĐH", "SoCT", "NgayĐH", "NgayGH", "SoPO", "MaKH", "TenKH", "CTKM", "CTĐG",
							...
							"QuyCach", "MaLH", "SoCont", "SoSeal", "SPChinh", "Huy", "Ghi chú"
					...
					for (OrderDetail it : orderDetails) {
						if (it.getProduct() != null) {
							...
							Object[] row = { it.getOrder_lix().getId(), it.getOrder_lix().getVoucher_code(),
									...
									it.isHuyct(), it.getOrder_lix().getNote() };
							results.add(row);
						}
					}
