...
public class InvoiceToJson {
	...
	private final static long EINVOICE_TYPE_MTT = 3;
	...
	public static String mabatbuoccodongiathanhtien[] = { "-", "$", "%", "&", "(", ")", "^", "+", "1", "5", "8", "A",
			"E", "P", "Q", "T", "V", "X", "Y" };
	private static final Logger LOGGER = Logger.getLogger(InvoiceToJson.class.getName());
...
//					}
//				}
				// Cài đặt ngày 17/06/2025
				for (int i = 0; i < hinhthucttkhongcaidat.length; i++) {
					if (hinhthucttkhongcaidat[i].equals(iec.getCode())) {
						v3.setPaymentMethod("Hàng không thu tiền");
						break;
					}
				}
				...
				if (invoiceLoad.getIdhoadongoc() != null && !"".equals(invoiceLoad.getIdhoadongoc().trim())) {
					Invoice invoiceLoadTT = invoiceService
							.findByIdSafe(Long.parseLong(invoiceLoad.getIdhoadongoc().trim()));
					...
					String newseriesyear_Old = ((invoiceLoadTT.getInvoice_date().getYear() + 1900) + "").substring(2,
							4);
				...
				/*
				 * Hóa đơn khi có số lượng buộc phải có đơn giá, thành tiền Cập nhật ngày
				 * 17/10/2023
				 */
				...
				List<InvoiceDetail> invoiceDetails = new ArrayList<>(listDetail);

				if (hdgomsanpham) {
				    invoiceDetails.clear();
				    
				    // 1. NHÓM THEO Product ID (sử dụng HashMap mặc định)
				    // Map<Product ID, List<InvoiceDetail>>
				    Map<Long, List<InvoiceDetail>> datagroupsById = listDetail.stream()
				        .collect(
				            Collectors.groupingBy(p -> p.getProduct().getId())
				        );

				    // 2. CHUYỂN THÀNH DANH SÁCH CÁC NHÓM (Map.Entry) ĐỂ SẮP XẾP
				    List<Entry<Long, List<InvoiceDetail>>> groupedEntries = 
				        new ArrayList<>(datagroupsById.entrySet());

				    // 3. SẮP XẾP DANH SÁCH CÁC NHÓM
				    // Sử dụng Comparator để sắp xếp:
				    // Sắp xếp dựa trên giá trị ID (InvoiceDetail.getId()) nhỏ nhất trong mỗi nhóm
				    groupedEntries.sort(Comparator.comparing(entry -> 
				        entry.getValue().stream()
				             .mapToLong(InvoiceDetail::getId) // ★ Lấy ID của từng dòng chi tiết
				             .min()                           // ★ Tìm ID nhỏ nhất trong nhóm
				             .orElse(Long.MAX_VALUE)          // Xử lý an toàn nếu List rỗng
				    ));

				    // 4. LẶP QUA CÁC NHÓM ĐÃ SẮP XẾP VÀ TÍNH TỔNG
				    for (Entry<Long, List<InvoiceDetail>> entry : groupedEntries) {
				        List<InvoiceDetail> invs = entry.getValue();
				        
				        double soluong = 0;
				        double tongtien = 0;
				        
				        // Tính tổng số lượng và tổng tiền trong nhóm
				        for (InvoiceDetail inv : invs) {
				            soluong += inv.getQuantity();
				            tongtien += inv.getTotal();
				        }
				        
				        // Tạo đối tượng InvoiceDetail mới đại diện cho nhóm đã tổng hợp
				        ...
				        // Thêm vào danh sách cuối cùng theo thứ tự đã sắp xếp (min ID)
				...
				for (InvoiceDetail p : invoiceDetails) {
					/*
					 * Cập nhật bổ sung 17/10/2023 Nếu hóa đơn điều chỉnh, hóa đơn chiết khấu (dòng
					 * chiết khấu), hóa đơn có mã xuất nhập Y và mã số thuế KH khác nội bộ
					 * (0301444263*) thì không kiểm tra Hóa đơn chiết khấu số lượng đang là 0 nên
					 * không kiểm tra.
					 */
					if (buoccodongiathanhtien) {
						if (hoadondieuchinh || ("Y".equals(iec.getCode()) && v3.getAccountObjectTaxCode() != null
								&& !v3.getAccountObjectTaxCode().contains("0301444263"))) {
							...
							if (p.getQuantity() != 0) {
								...
								if (hdxuatkhau == false) {
									if (Math.round(p.getUnit_price() * p.getQuantity()) != p.getTotal()) {
										// Hóa đơn không được chuyển
										thongbao.setLoi(true);
										thongbao.setThongtinloi(
												"Vui lòng kiểm tra lại dữ liệu phiếu " + invoiceLoad.getVoucher_code()
														+ ": Mã sản phẩm (" + p.getProduct().getProduct_code()
					...
					/**
					 * Ngay 31/03/2023 NHK dieu chinh lai cho phep the hien so (-) tren hoa don
					 */
					if (hdkhuyenmai && invoiceLoad.getTax_value() == 0) {
						...
						if (hdxuatkhau) {
							...
							itemv3.setVATAmountOC(
									MyMath.round(invoiceDetailLoad.getTotal() * invoiceLoad.getTax_value()));
						} else {
							itemv3.setAmountOC(itemv3.getAmount());
					...
					itemv3.setInventoryItemNote(null);
					/*
					 * Cập nhat them trang thai chiet khau (ngay 13/07/2023) CKN: CP CHIẾT KHẤU
					 */
		...
		try {
			...
			if (invoiceLoad != null) {
				...
				v3.setContractOwner(configEInvoice.getTencongty());// ten cong
																	// tu
				v3.setTransporterName(
						invoiceLoad.getCarrier() == null ? "" : invoiceLoad.getCarrier().getCarrier_name());
				...
				v3.setDeliveryType("1");// Ki?u phi?u Giá tr?: 1: phi?u n?i b?,
										// 2: phi?u d?i lý 1
				v3.setTransportContractCode(invoiceLoad.getFreight_contract() == null ? ""
						: invoiceLoad.getFreight_contract().getContract_no());
	...
	public static ThongBao toJsonHoaDon2(Invoice invoiceLoad, EInvoiceService eInvoiceService,
			IInvoiceService invoiceService, IInvoiceDetailService invoiceDetailService,
			IIEInvoiceService ieInvoiceService, boolean lamtron, String chinhanhapdung, String userAction,
			IProductService productService) {
		ThongBao thongbao = new ThongBao();
		try {
			// Lấy config và data Invoice.
			ConfigEInvoice configEInvoice = eInvoiceService.getConfigEInvoice();
			EInvoiceData eInvoiceData = eInvoiceService.selectEInvoiceDataById(EINVOICE_TYPE_MTT);
			if (configEInvoice == null || eInvoiceData == null) {
				thongbao.setLoi(true);
				thongbao.setThongtinloi("Không lấy được thông tin cấu hình.");
				return thongbao;
			...
			if (invoiceLoad != null) {
				// Kiểm tra và gán refId
				String refID = invoiceLoad.getRefId();
				String orderId = invoiceLoad.getOrder_voucher() != null ? invoiceLoad.getOrder_voucher() : "Unknown";
				String voucherCode = invoiceLoad.getVoucher_code() != null ? invoiceLoad.getVoucher_code() : "Unknown";
				if (refID == null || refID.trim().isEmpty()) {
					refID = UUID.randomUUID().toString();
					invoiceLoad.setRefId(refID);
					try {
						invoiceService.updateRefId(invoiceLoad.getId(), refID, userAction);
						LOGGER.info("Đã gán refId mới: " + refID + " cho hóa đơn orderId: " + orderId
								+ ", voucher_code: " + voucherCode);
						// Kiểm tra lại refId sau khi cập nhật
						Invoice updatedInvoice = invoiceService.findByIdSafe(invoiceLoad.getId());
						if (updatedInvoice == null || updatedInvoice.getRefId() == null
								|| updatedInvoice.getRefId().trim().isEmpty()) {
							LOGGER.warning("Hóa đơn vẫn thiếu refId sau khi cập nhật cho orderId: " + orderId
									+ ", voucher_code: " + voucherCode
									+ ". Có thể do lỗi giao dịch hoặc trùng lặp đơn hàng.");
						}
					} catch (Exception e) {
						LOGGER.warning("Lỗi khi cập nhật refId cho orderId: " + orderId + ", voucher_code: "
								+ voucherCode + ", error: " + e.getMessage());
					}
				}
				...
				if (listDetail == null || listDetail.isEmpty()) {
					thongbao.setLoi(true);
					thongbao.setThongtinloi("Không có chi tiết hóa đơn cho orderId: " + orderId);
					LOGGER.warning(
							"Không có chi tiết hóa đơn cho orderId: " + orderId + ", voucher_code: " + voucherCode);
					return thongbao;
				}
				Customer customer = invoiceLoad.getCustomer();
				List<EInvoiceV3> v3s = new ArrayList<>();
				EInvoiceV3 v3 = new EInvoiceV3();

				// Cập nhật editVersion nếu refID đã tồn tại
...
//				}
				...
				if (hdkhuyenmai && invoiceLoad.getTax_value() == 0.0) {
					v3.setTotalSaleAmountOC(0);
					v3.setTotalSaleAmount(0);
					v3.setTotalDiscountAmountOC(0);
					v3.setTotalDiscountAmount(0);
					...
					if (hdxuatkhau) {
						...
						v3.setTotalSaleAmountOC(tongtiennt);
						v3.setTotalVATAmountOC(tienthuent);
//						v3.setTotalAmountOC(tongtiennt);
//						v3.setTotalAmountWithVATOC(tongtiennt);
						v3.setTotalAmountOC(tongtiennt + tienthuent);
						v3.setTotalAmountWithVATOC(tongtiennt + tienthuent);
				...
				if (hdgomsanpham) {
					invoiceDetails.clear();
					Map<Product, List<InvoiceDetail>> datagroups1 = listDetail.stream()
