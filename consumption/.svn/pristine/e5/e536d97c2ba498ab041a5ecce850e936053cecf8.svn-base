...
public class PricingProgramBean extends AbstractBean {
	...
	private Date ngaybaogiadenngay;
	...
	public void exportPDF() {
		...
		try {
			if (listPricingProgramDetail != null && listPricingProgramDetail.size() > 0) {
				...
				String tgad = "";
				if (pricingProgramCrud.getExpiry_date() != null) {
					tgad = "từ ngày "
							+ ToolTimeCustomer.convertDateToStringShort(pricingProgramCrud.getEffective_date())
							+ " đến ngày "
							+ ToolTimeCustomer.convertDateToStringShort(pricingProgramCrud.getExpiry_date()) + ".";
				} else {
					tgad = "từ " + ToolTimeCustomer.convertDateToStringShort(pricingProgramCrud.getEffective_date())
				...
				for (int i = 0; i < listPricingProgramDetail.size(); i++) {
					List<ProductKM> productKMs = productKMService.findByIdProductMain(listPricingProgramDetail.get(i)
							.getProduct().getId());
//					if (productKMs.size() == 1) {
//						listPricingProgramDetail.get(i).setNote(
//								productKMs.get(0).getPromotion_product().getProduct_name());
//					}
					if (!productKMs.isEmpty()) {
						for (int j = 0; j < productKMs.size(); j++) {
							if(productKMs.get(j).isSpchinh()==false){
								listPricingProgramDetail.get(i).setNote(
										productKMs.get(j).getPromotion_product().getProduct_name());
								break;
							}
						}
					}
