...
//	q$(document).on('keyup', '.fixtoggel input.ui-column-filter', function() {
//		updateOnlyDatatable();
//	});
	q$(".ui-dialog").draggable();
...
// }
		...
		checkCookie:function(cname){
			...
			try{
	            ...
	            for (let i = 0; i < rowGroups.length; i ++) {
	            	let pa=q$(rowGroups[i]).closest("tr.ui-expanded-row");
	            	if(pa){
		...
		},expandTableByIndex:function(index,clazz){
			try{
				setTimeout(function(){
					clazz=clazz.replace(/(:|\.|\[|\]|,|=|@)/g, '\\$1');
					clazz=clazz+" tr.ui-widget-content[data-ri='"+index+"']";
					let rowGroups=q$(clazz);
					q$(rowGroups).find('.ui-row-toggler').trigger('click');
				}, 100);
			}catch(ex){
			}
			
		},showPDFK:function(base64,id){
			var options = { forceIframe: true };
			 var blobURL = URL.createObjectURL( pdfBlobConversion(base64, 'application/pdf'));
			var myPDF = PDFObject.embed(blobURL, "#example1", options);
		}
	};
}();
