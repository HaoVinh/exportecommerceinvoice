		...
		try {
			...
			cq.select(
					cb.construct(YeuCauKiemTraHangDetailDTO.class, root_.get("id"),product_.get("product_code"),
							...
							root_.get("kiemtradat"),root_.get("tieuchuan"),root_.get("nguyennhan"),root_.get("huonggiaiquyet"),root_.get("giahanluukho"))).where(cb.equal(root_.get("yeuCauKiemTraHang").get("id"), importId));
