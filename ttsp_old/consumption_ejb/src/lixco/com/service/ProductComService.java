...
public class ProductComService implements IProductComService {
	...
	public int search(String json, List<ProductCom> list) {
		int res = -1;
		try {/*
			 * { pcom_info:{pcom_code:'',pcom_name:'',product_brand_id:0},
	...
	public int findLike(String text, int pageSize, List<ProductCom> list) {
		...
		try {
			...
			dis.getExpressions().add(
					cb.like(cb.function("replace", String.class, cb.lower(root.get("pcom_name")), cb.literal("đ"),
	...
	public int complete(String text, List<ProductCom> list) {
		...
		try {
			...
			Root<ProductCom> root = cq.from(ProductCom.class);
			Predicate dis = cb.disjunction();
			dis.getExpressions().add(cb.equal(root.get("pcom_code"), text));
			dis.getExpressions().add(cb.like(root.get("pcom_name"), "%" + text + "%"));
			// (long id, String code, String name)
			cq.select(cb.construct(ProductCom.class, root.get("id"), root.get("pcom_code"), root.get("pcom_name")))
					.where(dis);
			TypedQuery<ProductCom> query = em.createQuery(cq);
			list.addAll(query.getResultList());
			res = 0;
