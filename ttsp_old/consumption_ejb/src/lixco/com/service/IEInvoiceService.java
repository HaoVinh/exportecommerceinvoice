...
public class IEInvoiceService implements IIEInvoiceService {
	...
	public int selectCommercialInvoiceReport(long iEInvoiceId, List<IECommercialInvoiceReportInfo> list) {
		...
		try {
			...
			sql.append("select p.id as product_id,p.product_code as product_code,p.en_name as product_en_name,p.specification,p.unit,pt.id as product_type_id,pt.en_name as product_type_en_name,t.quantity_export, ");
			...
			for (Object[] p : listObj) {
				IECommercialInvoiceReportInfo item = new IECommercialInvoiceReportInfo(Long.parseLong(Objects.toString(
						p[0], "0")), Objects.toString(p[1], null),Objects.toString(p[2], null), Double.parseDouble(Objects.toString(p[3])),
						Objects.toString(p[4], null), Long.parseLong(Objects.toString(p[5], "0")), Objects.toString(
								p[6], null), Double.parseDouble(Objects.toString(p[7], "0")),
						Double.parseDouble(Objects.toString(p[8], "0")),
						Double.parseDouble(Objects.toString(p[9], "0")), Objects.toString(p[10], ""));
				list.add(item);
			}
			res = 0;
		} catch (Exception e) {
		...
		try {/* list_ie_invoice_detail_id:[]} */
			...
			for (Object[] p : listObj) {
				IECommercialInvoiceReportInfo item = new IECommercialInvoiceReportInfo(Long.parseLong(Objects
						...
						Double.parseDouble(Objects.toString(p[3], "0")), Objects.toString(p[4], null),
						Long.parseLong(Objects.toString(p[5], "0")), Objects.toString(p[6], "0"),
		...
		try {
			...
			cq.select(
					cb.construct(IEInvoiceCustomerBillNoDetailReport.class, root.get("container_no"),
							...
							product.get("factor"), root.get("quantity_export")))
					.where(root.get("id").in(param))
					.orderBy(cb.asc(productType.get("en_name")), cb.asc(product.get("en_name")),
