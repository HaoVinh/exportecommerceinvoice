...
public class InvoiceService implements IInvoiceService {
	...
	public List<InvoiceTemp> findAllTemp(Date startMonth, Date endMonth) {
		...
		Date endDate = endMonth != null ? ToolTimeCustomer.getFirstDateOfDay(endMonth)
				: ToolTimeCustomer.getFirstDateOfDay(new Date());
	...
	public int prepareDataDelInvoice(long invoiceId, List<String> list) {
		...
		try {
			...
			cq.multiselect(root.get("idfox"), invoiceOwn.get("id"), invoiceOwn.get("idfox"))
					.where(cb.equal(root.get("id"), invoiceId));
		...
		try {
			...
			if (exOrder) {
				if (ipromotion) {
					...
					cq2.select(root2.get("idfox"))
							.where(cb.or(cb.equal(root2.get("invoice_detail_own").get("id"), idInvoiceDetailMain),
					...
					cq3.select(root3.get("idfox"))
							.where(cb.equal(root3.get("invoice_detail_own").get("id"), invoiceDetailId));
	...
	public int getListDataInvoiceFox(List<Long> listIdInvoice, List<lixco.com.reqfox.Invoice> list) {
		...
		try {
			...
			// idfox)
			cq.select(cb.construct(lixco.com.reqfox.Invoice.class, root_.get("id"), ieCate_.get("code"),
					root_.get("voucher_code"), customer_.get("customer_code"), root_.get("invoice_date"),
					warehouse_.get("code"), car_.get("license_plate"), root_.get("tax_value"), pm_.get("method_code"),
					root_.get("exported"), contract_.get("voucher_code"), root_.get("invoice_serie"),
					root_.get("order_voucher"), dp_.get("place_code"), hb_.get("harbor_code"),
					stocker_.get("stocker_code"), stevedore_.get("old_code"), formUpGoods_.get("old_code"),
					root_.get("timeout"), promotionPro_.get("program_code"), carrier_.get("carrier_code"),
					root_.get("transport_content"), pricing_.get("program_code"), order_.get("idfox"),
					root_.get("movement_commands_no"), root_.get("delivery_date"), root_.get("content_qd"),
					root_.get("note"), root_.get("po_no"), root_.get("time_up_goods"), root_.get("refId"),
					root_.get("editVersion"), root_.get("lookup_code"), root_.get("idfox")))
			...
			for (lixco.com.reqfox.Invoice p : list) {
				...
				cqDetail.select(cb.construct(lixco.com.reqfox.InvoiceDetail.class, root2.get("id"),
						...
						root2.get("foreign_unit_price"), root2.get("total_foreign_amount"), root2.get("refDetailID"),
						root2.get("idfox"), invoice_.get("idfox"))).where(cb.equal(invoice_.get("id"), p.getId()));
	...
	public int getListDataInvoiceFoxByIEInvoice(long iEInvoiceId, List<lixco.com.reqfox.Invoice> list) {
		...
		try {
			...
			cq.select(cb.construct(lixco.com.reqfox.Invoice.class, root_.get("id"), ieCate_.get("code"),
					...
					stocker_.get("stocker_code"), stevedore_.get("old_code"), formUpGoods_.get("old_code"),
					root_.get("timeout"), promotionPro_.get("program_code"), carrier_.get("carrier_code"),
					root_.get("transport_content"), pricing_.get("program_code"), iEInvoice_.get("idfox"),
					root_.get("movement_commands_no"), root_.get("delivery_date"), root_.get("content_qd"),
					root_.get("note"), root_.get("po_no"), root_.get("time_up_goods"), root_.get("refId"),
					root_.get("editVersion"), root_.get("lookup_code"), root_.get("idfox")))
	...
	public int search(String json, List<Invoice> list) {
		...
		try {
			...
			dis2_1.getExpressions()
					.add(cb.equal(root_.get("customer").get("customer_types").get("id"), pCustomerTypeId));
			predicates.add(dis2_1);
			Predicate dis2_2 = cb.disjunction();
			dis2_2.getExpressions().add(cb.equal(pCustomerChannelId, 0));
			dis2_2.getExpressions().add(cb.equal(
					root_.get("customer").get("customer_types").get("customer_channel").get("id"), pCustomerChannelId));
			...
			query.setParameter(pCustomerChannelId,
					Long.parseLong(Objects.toString(hCustomerChannelId.getValue(), "0")));
	...
	public int reportPhieuXuatKho(long invoiceId, List<PhieuXuatKho> list) {
		...
		try {
			...
			// total_amount)
			cq.select(cb.construct(PhieuXuatKho.class, product_.get("id"), product_.get("product_code"),
					product_.get("product_name"), root_.get("note_batch_code"), product_.get("unit"),
					root_.get("quantity"), root_.get("unit_price"), root_.get("total")))
	...
	public int reportLenhDieuDong(long invoiceId, List<LenhDieuDong> list) {
		...
		try {
			...
			cq.select(cb.construct(LenhDieuDong.class, product_.get("id"), product_.get("product_code"),
	...
	public int reportPhieuNhapKho(long invoiceId, List<PhieuNhapKho> list) {
		...
		try {
			...
			cq.select(cb.construct(PhieuNhapKho.class, product_.get("id"), product_.get("product_code"),
					product_.get("product_name"), product_.get("unit"),
					cb.prod(root_.get("quantity"), product_.get("factor")), root_.get("quantity"),
					root_.get("unit_price"), cb.prod(root_.get("unit_price"), root_.get("quantity"))))
					.where(cb.equal(root_.get("invoice").get("id"), invoiceId));
	...
	public int reportPhieuXuatKhoKiemVCNB(String json, List<PhieuXuatKhoKiemVCNB> list) {
		...
		try {
			...
			Predicate con = cb.conjunction();
			con.getExpressions().add(cb.equal(invoice_.get("invoice_date"), ToolTimeCustomer
					.convertStringToDate(Objects.toString(hInvoiceDate.getValue(), null), "dd/MM/yyyy")));
			con.getExpressions().add(cb.equal(invoice_.get("customer").get("id"),
			...
			cq.select(
					cb.construct(PhieuXuatKhoKiemVCNB.class, invoice_.get("invoice_code"), product_.get("product_name"),
							product_.get("unit"), root_.get("quantity"), car_.get("license_plate")))
					.where(con);
	...
	public int selectByInvoiceCodeOnlyId(String invoiceCode, InvoiceReqInfo t) {
		...
		try {
			...
			Root<Invoice> root = cq.from(Invoice.class);
			ParameterExpression<String> pInvoiceCode = cb.parameter(String.class);
			cq.select(cb.construct(Invoice.class, root.get("id")))
					.where(cb.equal(root.get("invoice_code"), pInvoiceCode));
			TypedQuery<Invoice> query = em.createQuery(cq);
			query.setParameter(pInvoiceCode, invoiceCode);
			t.setInvoice(query.getSingleResult());
			res = 0;
	...
	public int getListInventory(List<Long> listProductId, Date deliveryDate, List<Object[]> listInventory) {
		...
		try {
			if (deliveryDate != null && listProductId != null && listProductId.size() > 0) {
				...
				sql.append(
						"select t1.product_id,sum(unit_opening_balance+unit_import_quantity-unit_export_quantity) as inve,sum(unit_opening_balance+unit_import_quantity-unit_export_quantity-cal_quantity) as inv_cal from( ");
				sql.append(
						"select iv.product_id,iv.closing_balance as unit_opening_balance, 0 as unit_import_quantity, 0 as unit_export_quantity,0 cal_quantity  from inventory as iv where iv.inventory_month=:m and iv.inventory_year=:y and iv.product_id in (:ls) ");
				...
				sql.append(
						"where dx_.delivery_date >= :fd and dtx_.product_id in (:ls) and dx_.exported=0 and  dx_.delivery_date>=:d ");
				sql.append(" ) as t1 ");
	...
	public int reportPhieuXuatKhoLever(long invoiceId, List<PhieuXuatKhoLever> list) {
		...
		try {
			...
			Root<InvoiceDetail> root_ = cq.from(InvoiceDetail.class);
			Join<InvoiceDetail, Product> product_ = root_.join("product", JoinType.INNER);
			cq.select(cb.construct(PhieuXuatKhoLever.class, product_.get("id"), product_.get("lever_code"),
					product_.get("product_name"), cb.quot(root_.get("quantity"), product_.get("specification")),
					root_.get("note_batch_code"))).where(cb.equal(root_.get("invoice").get("id"), invoiceId));
	...
	public int updateTotalTax(Invoice invoice, String userAction) {
		...
		try {
			Query query = em.createQuery(
					"update Invoice set tongtien=:tt,thue=:th, act='TT-Thue', useraction=:useraction where id=:id");
	...
	public int updatePayment(Long cusid, Date date, String voucher_code, Date ngaythanhtoan, String userAction) {
		...
		try {
			Query query = em.createQuery(
					"update Invoice set payment=true, payment_date=:ntt,act='Thanhtoan', useraction=:useraction where customer_id=:cusid and invoice_date=:date and voucher_code=:vc");
	...
	public int updatePayment(long idinvoice, String userAction, boolean payment) {
		...
		try {
			Query query = em.createQuery(
					"update Invoice set payment=:payment,act='Thanhtoan', useraction=:useraction where id=:idinvoice");
	...
	public int updateJsonHoaDon(Invoice invoice, String userAction) {
		...
		try {
			Query query = em.createQuery(
					"update Invoice set jsonhoadon=:jsonhd,datehoadon=:datehd,act='jsonhoadon', useraction=:useraction where id=:id");
	...
	public int updateEditVersion(long id, int editVersion, String userAction) {
		...
		try {
			Query query = em.createQuery(
					"update Invoice set editVersion=:editVersion,act='editversion', useraction=:useraction where id=:id");
	...
	public int updateTongTienKM(long id, double total, String userAction) {
		...
		try {
			Query query = em.createQuery(
					"update Invoice set tongtien=:total,act='TongtienKM', useraction=:useraction where id=:id");
	...
	public int updateGetInvoice(long id, String lookup_code, String voucher_code, String userAction) {
		...
		try {
			...
			Query query = em.createQuery(
					"update Invoice set lookup_code=:lookup_code, voucher_code=:voucher_code, status=1,act='SoHDMaTC', useraction=:useraction where id=:id");
