...
//			cq.multiselect(cb.coalesce(cb.max(root.get("id")),0));
			cq.multiselect(cb.coalesce(cb.max(cb.quot((Expression)cb.substring(root.get("code"),2),1)),0));
...
//			logger.error("WarehouseService.initCode:"+e.getMessage(),e);
	...
	@Override
	public Warehouse selectByCodeOld(String code) {
		try{
			Query query=em.createQuery("select new Warehouse(id)  from Warehouse where old_code=:cd");
	...
	@Override
	public int complete(String text, List<Warehouse> list) {
		int res=-1;
		try{
			CriteriaBuilder cb=em.getCriteriaBuilder();
			CriteriaQuery<Warehouse> cq=cb.createQuery(Warehouse.class);
