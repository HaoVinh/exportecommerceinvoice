...
public class CustomerChannelService implements ICustomerChannelService {
	...
	@Override
	public int findLike(String text, List<CustomerChannel> list) {
		int res = -1;
		try {
			...
			cq.select(root).where(cb.like(root.get("name"), "%"+text+"%"));
	...
	@Override
	public CustomerChannel findByName(String name) {
		try {
			if (name != null && !"".equals(name)) {
				...
				TypedQuery<CustomerChannel> query = em.createQuery(cq);
				List<CustomerChannel> channels = query.getResultList();
				if (channels.size() != 0)
