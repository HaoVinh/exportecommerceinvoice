	...
	public List<Object[]> selectByInvoicesToDMS(int monthS, int yearS) {
		try {
			...
			sql.append("SELECT ").append("i.id as idinvoice, ").append("'BL146' as mact, ")
					...
					.append("ind.productdh_code as pdcode").append(" FROM ").append("invoice AS i ").append("JOIN ")
		...
		try {
			if (invoiceIds.size() != 0) {
				...
				sql.append("SELECT ").append("'BL146', ").append("i.voucher_code, ").append("i.invoice_date, ")
						...
						.append(" '', ").append("pdkm.product_code as mspkm, ").append("pdkm.factor as hsqdkm, ").append("sto.stocker_name as stoName ");
				sql.append("FROM ").append("invoice AS i ").append("JOIN ")
						...
						.append("product AS pdkm ON pdkm.id = pd.promotion_product_id ");
