...
public class CountryService implements ICountryService {
	...
	@Override
	public int search(String json,  List<Country> list) {
		int res = -1;
		try {
	...
	public int complete(String text, List<Country> list) {
		...
		try {
			...
			dis.getExpressions().add(
					cb.like(cb.function("replace", String.class, cb.lower(root.get("country_name")), cb.literal("đ"),
							cb.literal("d")), "%" + text + "%"));
			cq.select(cb.construct(Country.class, root.get("id"), root.get("country_code"), root.get("country_name")))
					.where(dis);
			TypedQuery<Country> query = em.createQuery(cq);
			list.addAll(query.getResultList());
			res = 0;
