...
public class CarService implements ICarService {
	...
	public lixco.com.reqfox.Car getCarFoxPro(long carId) {
		try {
			...
			CriteriaQuery<lixco.com.reqfox.Car> cq = cb.createQuery(lixco.com.reqfox.Car.class);
			Root<Car> root = cq.from(Car.class);
			Join<Car, CarOwner> carOwn_ = root.join("car_owner", JoinType.LEFT);
			Join<Car, CarType> carType_ = root.join("car_type", JoinType.LEFT);
			// Car(String soxe, String tenxe, String nhomxe, String masochuxe,
			// String dienthoai,
			cq.select(
					cb.construct(lixco.com.reqfox.Car.class, root.get("id"), root.get("license_plate"),
							root.get("driver"), carType_.get("code"), carOwn_.get("code"), root.get("phone_number"),
							root.get("idfox"))).where(cb.equal(root.get("id"), carId));
			TypedQuery<lixco.com.reqfox.Car> query = em.createQuery(cq);
			return query.getSingleResult();
		} catch (Exception e) {
			// logger.error("CarService.getCarFoxPro:"+e.getMessage(),e);
		}
		return null;
	}
	...
	public int search(String json, List<Car> list) {
		...
		try {/*
			...
			dis1.getExpressions().add(cb.equal(pCarTypeId, 0));
	...
	public int complete(String text, List<Car> list) {
		...
		try {
			...
			CriteriaQuery<Car> cq = cb.createQuery(Car.class);
			Root<Car> root = cq.from(Car.class);
			cq.select(cb.construct(Car.class, root.get("id"), root.get("license_plate"), root.get("driver"))).where(
					cb.like(root.get("license_plate"), "%" + text + "%"), cb.equal(root.get("disable"), false));
			TypedQuery<Car> query = em.createQuery(cq);
			list.addAll(query.getResultList());
			res = 0;
		} catch (Exception e) {
	...
	public int complete2(String text, List<Car> list) {
		...
		try {
			...
			CriteriaQuery<Car> cq = cb.createQuery(Car.class);
			Root<Car> root = cq.from(Car.class);
			Join<Car, CarOwner> carOwner_ = root.join("car_owner", JoinType.INNER);
			cq.select(
					cb.construct(Car.class, root.get("id"), root.get("license_plate"), carOwner_.get("id"),
							carOwner_.get("name"))).where(cb.like(root.get("license_plate"), "%" + text + "%"),
					cb.equal(root.get("disable"), false));
			TypedQuery<Car> query = em.createQuery(cq);
			list.addAll(query.getResultList());
			res = 0;
		} catch (Exception e) {
			logger.error("CarService.complete2:" + e.getMessage(), e);
		}
		return res;
	}
	...
	public int updateIdFox(lixco.com.reqfox.Car car) {
		int res = -1;
		try {
			Query query = em.createQuery("update Car set idfox=:f where id=:id ");
			query.setParameter("f", car.getIdfox());
			query.setParameter("id", car.getId());
			res = query.executeUpdate();
		} catch (Exception e) {
			logger.error("CarService.updateIdFox:" + e.getMessage(), e);
		}
		return res;
	}
