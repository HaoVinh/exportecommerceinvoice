...
public class ReportHangChamLuanChuyenBean extends AbstractBean {
	...
	public void hangChamLuanChuyenExcel() {
		try {
			 ...
			 * lay ton kho thang
			 */
			List<TonKhoThang> tonkhothangs = inventoryService.tonkhothang(JsonParserUtil.getGson().toJson(json));
			...
			for (int i = 0; i < tonkhothangs.size(); i++) {
				if (tonkhothangs.get(i).getKg_closing_balance() != 0) {
					...
					hcLuanChuyen.setNam(yearSearch);
					hcLuanChuyen.setThang(monthSearch);
			...
			if (hangChamLuanChuyens.size() > 0) {
				...
				LocalDate date1 = new LocalDate(yearSearch, monthSearch, 1).dayOfMonth().withMaximumValue();
				for (int i = 0; i < hangChamLuanChuyens.size(); i++) {
					...
					Object[] row = { hangChamLuanChuyens.get(i).getMasp(), hangChamLuanChuyens.get(i).getTensp(),
							hangChamLuanChuyens.get(i).getSlton(), hangChamLuanChuyens.get(i).getKho(), Math.abs(numberOfMonths),
							hangChamLuanChuyens.get(i).getNgaysxgannhat(), hangChamLuanChuyens.get(i).getLoaisp() };
					results.add(row);
				}

				StringBuilder title2 = new StringBuilder();
				title2.append("HangChamLuanChuyen_" + monthSearch + "_" + yearSearch);
