	...
	public void reportBangKeKhoiLuongVanChuyenExcel(){
		...
		try{
			...
			if(list.size()>0){
				...
				Object[] title={"SỐ XE","NGÀY","MÃ KH","TÊN KHÁCH HÀNG","MÃ TP","THÀNH PHỐ","SỐ HÓA ĐƠN","LOẠI SẢN PHẨM","NHÓM","SỐ LƯỢNG"};
				listResult.add(title);
				for(BangKeKhoiLuongVanChuyen p:list){
					Object[] row={p.getLicense_plate(),ToolTimeCustomer.convertDateToString(p.getInvoice_date(),"dd/MM/yyyy"),p.getCustomer_code(),
