...
//		try {
//			List<InvoiceDetail> listInvoiceTempDetail = new ArrayList<>();
//			invoiceDetailService.selectByInvoice(invoiceCrud.getId(), listInvoiceTempDetail);
//			double totalevent = listInvoiceTempDetail.stream().mapToDouble(f -> f.getTotal()).sum();
//			invoiceCrud.setTongtien(MyMath.round(totalevent));
//			if (invoiceCrud.getTax_edit() != 0) {
//				invoiceCrud.setThue(invoiceCrud.getTax_edit());
//			} else {
...
//			current.executeScript("PF('tablect').clearFilters();");
//			if (wrapInvoiceDetailSelect != null) {
//				if (allowDelete(wrapInvoiceDetailSelect.getInvoice_detail().getInvoice().getInvoice_temp_date(),
...
//					if (detail != null) {
//						if (detail.getInvoice() != null && allowDelete(detail.getInvoice().getInvoice_temp_date(),
...
//							it.getInvoice().getInvoice_temp_date(), cus, wah, ie, ct, it.getProduct().getProduct_code(),
		...
		try {
			if (allowDelete(wrapInvoiceDetailSelect.getInvoice_detail().getInvoicetemp().getInvoice_temp_date(),
				...
				int code = processLogicInvoiceService
						.deleteInvoiceDetailTempMaster(t1, message, getAccount().getUserName());
				if (code == 0) {
					listWrapInvoiceDetail.remove(wrapInvoiceDetailSelect);
					notify.success("Xóa thành công!");
//					sumInvoice();// tinh lai tong tien
			...
			if (invoiceCrud != null && invoiceCrud.getId() != 0) {
				if (allowUpdate(null)) {
					...
					if (listWrapInvoiceDetail != null && listWrapInvoiceDetail.size() > 0) {
						for (WrapInvoiceDetailTempReqInfo w : listWrapInvoiceDetail) {
							if (w.getInvoice_detail().getReal_quantity() > 0 && w.getInvoice_detail().getProductdh_code() == null) {
								listData.add(w);
							}
						}
					}
...
//						promotionOrderDetailService.selectByOrderTemp(invoiceCrud.getOrder_lix().getId(), listPromotionOrderDetailTemp);
						WrapInvoiceDetailTempReqInfo data = new WrapInvoiceDetailTempReqInfo(invoiceCrud.getId(), account
						...
						List<Long> listIdResult = new ArrayList<>();
						HeThong ht = heThongService.findById(1);
						int code = processLogicInvoiceService.createInvoiceByTemp(data, listIdResult,
					...
					if(t.getInvoice_detail().getProductdh_code() == null)
					{
						if (t.getInvoice_detail().getReal_quantity() > 0) {
							if (invoiceCrud.getOrder_lix().getPromotion_program() != null) {
								...
								js.addProperty("promotion_program_id",
										invoiceCrud.getOrder_lix().getPromotion_program() != null ? invoiceCrud.getOrder_lix().getPromotion_program()
												.getId() : 0);
								...
								lv1: for (PromotionProgramDetail ppd : list) {
									...
									json.addProperty(
											"date",
											ToolTimeCustomer.convertDateToString(t.getInvoice_detail().getInvoicetemp()
													.getInvoice_temp_date(), "dd/MM/yyyy"));
									json.addProperty("product_id", ppd.getPromotion_product().getId());
									...
									double quantity = (int) t.getInvoice_detail().getReal_quantity_box() / ppd.getBox_quatity()
											* ppd.getSpecification();
									pod.setQuantity((int) quantity);
