...
public class IECategoriesBean extends AbstractBean  {
	...
	public void exportExcel() {
		try {
			if (listIECategories.size() > 0) {
				List<Object[]> results = new ArrayList<Object[]>();
				Object[] title = { "id", "maxn", "noidung", "ma(cu)" };
				results.add(title);
				for (IECategories p : listIECategories) {
					Object row[] = { p.getId(), p.getCode(), p.getContent(), p.getOld_code()};
					results.add(row);
				}
				String titleEx = "dmxuatnhap";
				...
				if (code != null && !"".equals(code) && content != null && !"".equals(content)) {
					...
					if (ieCategoriesCrud.getId() == 0) {
						...
						if (allowSave(new Date())) {
							...
							default:
								current.executeScript(
										"swaldesigntimer('Xảy ra lỗi!', 'Thêm thất bại!','warning',2000);");
								break;
							}
						} else {
							current.executeScript(
						...
						if (allowUpdate(new Date())) {
							...
							default:
								current.executeScript(
										"swaldesigntimer('Xảy ra lỗi!', 'Cập nhật thất bại!','error',2000);");
								break;
							}
						} else {
							current.executeScript(
					...
					IECategories lix = new IECategories();
					 while (cells.hasNext()) {
						...
						case 0:
							try {
								//mã nhập xuất old
								String maold = Objects.toString(MyUtilExcel.getCellValue(cell), null);
						...
						case 1:
							try {
								//Nội dung
								String content = Objects.toString(MyUtilExcel.getCellValue(cell), null);
