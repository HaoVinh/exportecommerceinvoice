...
public class InvoiceTempServlet {
	...
	public Response getInvoiceTemp(@QueryParam(DefinedName.PARAM_COMMAND) String cmd,
			...
			@QueryParam(DefinedName.PARAM_EDATE) String toDateStr) throws IOException, ServletException {
		String result = "";
		ResponseBuilder responseBuilder = Response.status(Response.Status.BAD_REQUEST);
		try {
			SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
			Date ngayNhap = dateFormat.parse(fromDateStr);
			Date toNgayNhap = dateFormat.parse(toDateStr);
			StringBuilder contentBuilder = new StringBuilder();
			if (cmd != null && fromDateStr != null) {
				invoiceTempHandler.process(cmd, responseBuilder, contentBuilder, ngayNhap, toNgayNhap);
			} else {
			...
			logger.error("InvoiceTempServlet.postAction:" + e.getMessage(), e);
