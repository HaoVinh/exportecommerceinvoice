					...
					for (int i = 0; i < listProcessContract.size(); i++) {
						...
						ContractLiquidationReqInfo item = new ContractLiquidationReqInfo(
								...
								productType != null ? productType.getEn_name() : "", listProcessContract.get(i)
										...
										.getContract_detail().getProduct().getEn_name(), Math.round(listProcessContract.get(i)
										.getDathuchien()
										/ listProcessContract.get(i).getContract_detail().getProduct().getFactor()
										/ listProcessContract.get(i).getContract_detail().getProduct()
												.getSpecification()), MyMath.roundCustom(listProcessContract.get(i).getDathuchien()
										/ listProcessContract.get(i).getContract_detail().getProduct().getFactor(),2), listProcessContract.get(i).getContract_detail().getUnit_price(),
								listProcessContract.get(i).getTiendathuchien(), enUnit);
						listData.add(item);
						if (enUnit.equals("MTS")) {
							mts += item.getEn_quantity();
							item.setEn_quantity(MyMath.roundCustom(item.getEn_quantity()/1000,2));
