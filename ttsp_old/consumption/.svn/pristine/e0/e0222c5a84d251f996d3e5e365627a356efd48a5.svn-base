...
public class UserRightData2Service {
	public static final String NAME = "userright2";
	static Gson gson;
	static {
	...
	/**
	 * 
	 * @param method tai chuong trinh
	 * @param param idaccount,idaccount
	 * @return Chuong trinh
	 */
	public static UserRightData2[] dsform(long idaccount,long idprogram) {
		try {
			String param=idaccount+","+idprogram;
			String link = "?cm=dsform&dt=" + param;
			String data = process(link);
			DataResponseAPI ketqua = gson.fromJson(data, DataResponseAPI.class);
			UserRightData2[] userRightDatas = gson.fromJson(ketqua.getDt(), UserRightData2[].class);
			return userRightDatas;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
