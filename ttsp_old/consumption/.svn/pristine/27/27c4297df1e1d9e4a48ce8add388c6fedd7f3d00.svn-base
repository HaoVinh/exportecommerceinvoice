...
public class YeuCauKiemTraHangTHService {
	...
	public List<YeuCauKiemTraHangDTO> searchAPI(Date startDate, Date endDate) {
		try {
			...
			predicates.add(cb.equal(root_.get("kcsdatiepnhan"), true));
			cq.select(
					cb.construct(YeuCauKiemTraHangDTO.class, root_.get("id"), root_.get("createdBy"),
							...
							root_.get("tpxacnhan"), root_.get("tpxacnhanDate"), root_.get("tpxacnhanName"),
							root_.get("kcsdatiepnhan"), root_.get("ngaytiepnhan"), root_.get("nguoitiepnhan"),
							root_.get("dakiemtra"), root_.get("ngayKiemTra"), root_.get("createdCheck"),
							root_.get("bldxacnhan"), root_.get("bldxacnhanDate"), root_.get("tpkcsxacnhankq"),
							root_.get("tpkcsxacnhankqDate"), root_.get("tpkcsxacnhankqName")))
			...
			if (p != null) {
				...
				if (p.getId() != 0) {
					...
					if (yeuCauKiemTraHang.isDakiemtra()) {
						messages.setUser_message("Phiếu đã kiểm tra, không cập nhật được.");
						ut.rollback();
						return res;
					}
					// cập nhật phiếu nhập
							...
							if (dt != null) {
								...
								if (product != null) {
									...
									if (dt.getId() == 0) {
										...
										YeuCauKiemTraHangDetail dtOld = yeuCauKiemTraHangDetailService.findById(dt
												.getId());
										dtOld.setLohang(dt.getLohang());
										dtOld.setQuantity(dt.getQuantity());
										dtOld.setNguongoc(dt.getNguongoc());
										dtOld.setTinhtrang(dt.getTinhtrang());
										dt = em.merge(dtOld);
										if (dt == null) {
											res = -1;
