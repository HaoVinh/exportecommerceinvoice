...
public class LuanChuyenServlet {
	...
	public Response findHangChamLuanChuyen(@QueryParam(DefinedName.PARAM_JSON) String paramJson,
		...
		try {
			...
			lixco.com.common.HolderParser hMonth = JsonParserUtil.getValueNumber(jo, "month", null);
			lixco.com.common.HolderParser hYear = JsonParserUtil.getValueNumber(jo, "year", null);
			List<HangChamLuanChuyen> hangChamLuanChuyens = new ArrayList<HangChamLuanChuyen>();

			List<TonKhoThang> tonkhothangCNs = inventoryService.tonkhothang(paramJson);
...
//				if (tonkhothangCNs.get(i).getKg_closing_balance() != 0) {
					...
					hcLuanChuyen.setNam(Integer.parseInt(Objects.toString(hMonth.getValue())));
					hcLuanChuyen.setThang(Integer.parseInt(Objects.toString(hYear.getValue())));
