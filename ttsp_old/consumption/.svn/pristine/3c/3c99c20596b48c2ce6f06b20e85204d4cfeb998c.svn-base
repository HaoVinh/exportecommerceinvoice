...
@WebServlet(name = "XNYeuCauServlet", urlPatterns = { "/XNYeuCauServlet/*" })
public class XNYeuCauServlet extends HttpServlet {
		...
		try {
			...
			if (sessionIdFromEmail != null && idphieu != null) {
				...
				if (acCf != null) {
					...
					if (yc != null) {
						Date now = new Date();
						session.setAttribute("tieude", "XÁC NHẬN PHIẾU YÊU CẦU KIỂM TRA HÀNG HÓA");
						...
						// TP Kho van xac nhan yeu cau
						if (yc.isTpxacnhan() == false) {
							yc.setTpxacnhan(true);
							yc.setTpxacnhanDate(now);
							yc.setTpxacnhanName(acCf.getNameEmp());
							yeuCauKiemTraHangService.updateTPXacNhan(yc);
							guiMailKCS(yc, request.getServletContext());
						}
	...
	static final String CONFIRM_YEUCAUKIEMTRA = "/TNYeuCauServlet";

	public void guiMailKCS(YeuCauKiemTraHang yeuCauKiemTraHangCrud, ServletContext servletContext) {
		...
		for (int i = 0; i < accountEmailSends.size(); i++) {
			...
			if (accountData != null) {
				...
				String linkConfirm = StaticPath.getLinkConfirmEmail() + CONFIRM_YEUCAUKIEMTRA + "?sessionIdFromEmail="
				...
				boolean result = MyMailUtil.sendMailConfirmYCKT(yeuCauKiemTraHangCrud, yeuCauKiemTraHangDetails,
						...
						new DataSendMail("", " tiếp nhận yêu cầu ", "", ""), servletContext,"Yêu cầu kiểm tra hàng hóa");
				if (result) {
					System.out.println("Đã gửi mail yêu cầu P. Quản lý chất lượng kiểm tra.");
				} else {
					System.out.println("Lỗi: Gửi mail yêu cầu kiểm tra không thành công.");
				}
			}
		}
	}
