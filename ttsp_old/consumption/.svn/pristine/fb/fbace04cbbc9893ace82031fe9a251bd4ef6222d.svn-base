...
public class CityBean extends AbstractBean {
	...
	public void search() {
		try {
			...
			cityService.search(JsonParserUtil.getGson().toJson(json),  listCity);
		} catch (Exception e) {
				...
				while (rows.hasNext()) {
					...
					lv2: while (cells.hasNext()) {
						...
						case 0:
							try {
								// mã thành phố
								String mtp = Objects.toString(MyUtilExcel.getCellValue(cell), null);
						...
						case 1:
							try {
								// tên thành phố
								String tentp = Objects.toString(MyUtilExcel.getCellValue(cell), null);
						...
						case 2:
							try {
								// mã khu vực
								String makv = Objects.toString(MyUtilExcel.getCellValue(cell), null);
						...
						case 6:
							try {
								// mã quốc gia
								String maqg = Objects.toString(MyUtilExcel.getCellValue(cell), null);
