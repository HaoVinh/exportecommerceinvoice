...
@Named
@ViewScoped
public class ReportTheKhoSanPhamDVTBean extends AbstractBean {
	...
	public void reportTheKhoSanPham() {
		...
		try {
			...
			reportService.reportTheKhoSanPhamDVT(JsonParserUtil.getGson().toJson(json), list);
...
//				}
				...
				for (Entry<Long, List<TheKhoSanPham>> entry : mapGroupByProductId.entrySet()) {
					...
					for (Entry<Integer, List<TheKhoSanPham>> entry1 : mapGroupByMonth.entrySet()) {
						...
						TonKhoDauThang tk = reportService.reportTonKhoDauThangDVT(thangtondau, namtondau, listGroupByMonth
								.get(0).getProduct_id());
						...
						for (int i = 0; i < listGroupByMonth.size(); i++) {
							...
							listGroupByMonth.get(i).setKg_ivn_quantity2(MyMath.roundCustom(listGroupByMonth.get(i).getKg_ivn_quantity()*listGroupByMonth.get(i).getFactor(),2));
				...
				String reportPath = FacesContext.getCurrentInstance().getExternalContext()
						.getRealPath("/resources/reports/reportinsanpham/the_kho_san_pham/the_kho_san_pham_dvt.jasper");
