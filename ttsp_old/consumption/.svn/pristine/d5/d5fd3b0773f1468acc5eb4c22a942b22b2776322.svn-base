			...
			if ("indonhangtonghop".equals(loaibaocao)) {
				...
				if (orderChooses.size() != 0) {
					...
					for (int i = 0; i < objspkhuyenmais.size(); i++) {
						SanPhamDonHang sp = new SanPhamDonHang();
						sp.setStt(i + 1);
						sp.setTensanpham(objspchinhs.get(i)[4]+" - "+objspkhuyenmais.get(i)[0]);
	...
	public void exportPDF() {
		...
		try {
			if (orderLixCrud != null && orderLixCrud.getId() > 0) {
				...
				double trongtaibb = 0;
				for (int i = 0; i < listOrderDetail.size(); i++) {
					System.out.println(listOrderDetail.get(i).getColumnsort() + " - "
							+ listOrderDetail.get(i).getIndexsort() + " - " + listOrderDetail.get(i).getId() + " - "
							+ listOrderDetail.get(i).getProduct().getProduct_code());
				...
				for (Product key : datagroups1.keySet()) {
					...
					trongtaibb += (prd.getQuantity() / key.getSpecification()) * key.getTare();
				}
