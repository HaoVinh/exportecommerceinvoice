...
public class CustomerBean extends AbstractBean {
	...
	private CustomerAsyncDTO dongbodata() {
		...
		// Khach hangf
		List<Customer> products = customerService.findNotSync();
		List<CustomerDTO> customerDTOs = new ArrayList<CustomerDTO>();
		for (int i = 0; i < products.size(); i++) {
			CustomerDTO pdDTO = new CustomerDTO(products.get(i));
			customerDTOs.add(pdDTO);
		}
		CustomerAsyncDTO customerAsyncDTO = new CustomerAsyncDTO(cityDTOs, customerChannelDTOs, customerTypesDTOs,
				customerDTOs);
	...
	// try{
	// lixco.com.reqfox.Customer
	// customer=customerService.getCustomerFoxPro(customerId);
	// AccountDatabase
	// accountDatabase=accountDatabaseService.findByName("foxproapi");
	// if(customer !=null){
	// try{
	// Call
	// call=ApiCallClient.getListObjectWithParam(accountDatabase.getAddressPublic(),"customer","sa",JsonParserUtil.getGson().toJson(customer));
	// Response response= call.execute();
	// String body=response.body().string();
	// JsonObject
	// result=JsonParserUtil.getGson().fromJson(body,JsonObject.class);
	// if(response.isSuccessful() && result.get("err").getAsInt()==0){
	// lixco.com.reqfox.Customer
	// customerResult=JsonParserUtil.getGson().fromJson(result.get("dt"),lixco.com.reqfox.Customer.class);
	//
	// }else{
	// String mesages=result.get("msg").getAsString();
	// current.executeScript(
	// "swaldesignclose2('Cảnh báo!', '"+mesages+"','warning');");
	// }
	// }catch (Exception e) {
	// e.printStackTrace();
	// current.executeScript(
	// "swaldesignclose2('Cảnh báo!', 'Không thực hiện liên kết dữ liệu foxpro','warning');");
	// }
	// }
	// }catch (Exception e) {
		...
		try {
			...
			if (list.size() > 0) {
				...
				Object[] title = { "MaKH", "tên khách hàng", "đơn vị", "địa chỉ", "IdLoaiKH", "tên loại khách hàng",
						...
						"mã số thuế", "địa chỉ giao hàng", "nghỉ bán" };
				results.add(title);
				for (Customer p : list) {
				...
				while (rows.hasNext()) {
					...
					lv2: while (cells.hasNext()) {
						...
						case 6:
							try {
								...
								if (cellvalue != null && !"".equals(cellvalue)) {
									...
									cityService.selectByCode(cellvalue, cf);
						...
						case 9:
							try {
								...
								if (cellvalue != null && !"".equals(cellvalue)) {
									...
									customerTypesService.selectByCode(cellvalue, ctf);
						...
						case 10:
							try {
								// mã số khách hàng
								lix.setCustomer_dfcode(cellvalue);
							} catch (Exception e) {
							}
							break;
						case 11:
							try {
								// số ngày nợ
								lix.setDays_debt_quantity(Double.parseDouble(cellvalue));
							} catch (Exception e) {
							}
							break;
						...
						case 13:
							try {
								// địa điểm giao hàng
								lix.setLocation_delivery(cellvalue);
							} catch (Exception e) {
							}
							break;
						case 14:
							try {
								...
								lix.setEncourage_rate(Double.parseDouble(cellvalue));
							} catch (Exception e) {
							}
							break;
						case 15:
							try {
								...
								lix.setCommission_value(Double.parseDouble(cellvalue));
							} catch (Exception e) {
							}
							break;
						case 16:
							try {
								// số tiền npp chuyển
								lix.setAmount_npp(Double.parseDouble(cellvalue));
							} catch (Exception e) {
							}
							break;
						case 17:
							try {
								// mã số nhà cung cấp
								lix.setSupplier_code(cellvalue);
							} catch (Exception e) {
							}
							break;
						case 18:
						...
						case 19:
							try {
								// hoa hồng ntrl
								lix.setCommission_ntrl_value(Double.parseDouble(cellvalue));
							} catch (Exception e) {
							}
							break;
						case 20:
							try {
								...
								lix.setNot_print_customer_name(Boolean.parseBoolean(cellvalue));
							} catch (Exception e) {
							}
							break;
						case 21:
