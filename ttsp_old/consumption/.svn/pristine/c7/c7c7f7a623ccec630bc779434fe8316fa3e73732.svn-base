		...
		try {
			...
			for (int m = 0; m < customers.size(); m++) {
				...
				if (list.size() > 0) {
					...
					if (invenCongNo.getInvenFirst() != 0) {
						SoDuCongNo item = new SoDuCongNo();
						item.setCustomer_code(cus.getCustomer_code());
						item.setCustomer_name(cus.getCustomer_name());
						item.setTotalFirst(invenCongNo.getInvenFirst());
						item.setTotalFinal(invenCongNo.getInvenFirst());
						soDuCongNos.add(item);
					}
	...
	public void soducongno2(List<Customer> customers) {
		...
		try {
			...
			if (list.size() > 0) {
				...
				List<InvenCongNo> tondaukys = invenCongNoService.getInvenFinal2(customers, thang, nam);
				for (int i = 0; i < list.size(); i++) {
					for (int j = 0; j < tondaukys.size(); j++) {
						if (list.get(i).getCustomer_code().equals(tondaukys.get(j).getCustomer().getCustomer_code())) {
							list.get(i).setTotalFirst(MyMath.round(tondaukys.get(j).getInvenFinal()));
							break;
						}
					}
					list.get(i).setTotalNo(MyMath.round(list.get(i).getTotalNo()));
				...
				for (int j = 0; j < tondaukys.size(); j++) {
					...
					if(status){
						double tondauky=tondaukys.get(j).getInvenFinal();
						if (tondauky != 0) {
							SoDuCongNo item = new SoDuCongNo();
							item.setCustomer_code(tondaukys.get(j).getCustomer().getCustomer_code());
							item.setCustomer_name(tondaukys.get(j).getCustomer().getCustomer_name());
							item.setTotalFirst(tondauky);
							item.setTotalFinal(tondauky);
							list.add(item);
						}
					}
				}
