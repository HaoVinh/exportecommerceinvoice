...
public class CustomerService implements ICustomerService {
	...
	public Customer findById(long id) {
		try {
			...
			Hibernate.initialize(cus.getCustomer_types());
	...
	public List<KhachHangData> complete2(String makh, String tenkh) {
		try {
			...
			cq.select(
					cb.construct(KhachHangData.class, root.get("customer_code"), root.get("customer_name"),
							concat(";", root.get("cell_phone"), root.get("email")), root.get("address"))).where(dis);
	...
	public Customer selectByCodeNotDisable(String code) {
		...
		try {
			...
			CriteriaQuery<Customer> cq = cb.createQuery(Customer.class);
			Root<Customer> root = cq.from(Customer.class);
			cq.select(root).where(cb.equal(root.get("customer_code"), code), cb.equal(root.get("disable"), false));
			TypedQuery<Customer> query = em.createQuery(cq);
			return query.getSingleResult();
		} catch (Exception e) {
			// logger.error("CustomerService.selectByCode:"+e.getMessage(),e);
	...
	public int dongbokhachhang(CustomerAsyncDTO customerAsyncDTO, StringBuilder errors) {
		...
		// Nhom khach hang
		List<CustomerGroupDTO> customerGroupDTOs = customerAsyncDTO.getCustomerGroupDTOs();
		for (CustomerGroupDTO it : customerGroupDTOs) {
			CustomerGroup customerGroupNew = new CustomerGroup(it);
			CustomerGroup pOld = customerGroupService.findByCode(it.getCode());
			if (pOld != null) {
				customerGroupNew.setId(pOld.getId());
				int result = customerGroupService.update(customerGroupNew);
				if (result != 0) {
					errors.append("Lỗi CustomerGroupDTO:" + it.getName() + ";");
				}
			} else {
				int result = customerGroupService.insert(customerGroupNew);
				if (result != 0) {
					errors.append("Lỗi CustomerGroupDTO:" + it.getName() + ";");
				}
			}
		...
		for (CustomerTypesDTO it : customerTypesDTOs) {
			...
			customerTypesNew.setCustomerGroup(customerGroupService.findByName(it.getNameCustomer_group()));
