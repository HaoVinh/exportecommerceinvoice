...
public class InvoiceBean extends AbstractBean {
	...
	public void search() {
		try {
			...
			jsonInfo.addProperty("customer_channel_id", customerChannelSearch == null ? 0 : customerChannelSearch.getId());
	...
	public void capnhatiddonhang() {
		...
		for (int i = 0; i < listInvoice.size(); i++) {
			if (listInvoice.get(i).getIdorderlix() == 0) {
				List<InvoiceDetail> listInvoiceDetail = new ArrayList<>();
				...
				if (idchitietdonhang != 0) {
					...
					if (od != null && od.getInvoice().getIdorderlix() != 0) {
						listInvoice.get(i).setOrder_voucher(od.getInvoice().getOrder_voucher());
						listInvoice.get(i).setIdorderlix(od.getInvoice().getIdorderlix());
						invoiceService.updateDonHang(listInvoice.get(i));
						sodonhangcnduoc++;
					}
				}
			}
	...
	public void capnhatiddonhang2() {
		int sodonhangcnduoc = 0;
		for (int i = 0; i < listInvoice.size(); i++) {
			if (listInvoice.get(i).getOrder_voucher() != null && !"".equals(listInvoice.get(i).getOrder_voucher())
					&& listInvoice.get(i).getIdorderlix() == 0) {
				List<OrderLix> orderLixs = orderLixService.selectByOrderVoucher(listInvoice.get(i).getOrder_voucher(),
						listInvoice.get(i).getCustomer().getId());

				if (orderLixs.size() == 1) {
					listInvoice.get(i).setOrder_voucher(orderLixs.get(0).getVoucher_code());
					listInvoice.get(i).setIdorderlix(orderLixs.get(0).getId());
					invoiceService.updateDonHang(listInvoice.get(i));
					sodonhangcnduoc++;
				}
			}
	...
	public void capnhatsodonhang() {
		...
		for (int i = 0; i < listInvoice.size(); i++) {
			if (listInvoice.get(i).getIdorderlix() != 0) {
				OrderLix orderLix = orderLixService.findById(listInvoice.get(i).getIdorderlix());
				if (orderLix != null) {
					listInvoice.get(i).setOrder_voucher(orderLix.getVoucher_code());
					listInvoice.get(i).setIdorderlix(orderLix.getId());
					invoiceService.updateDonHang(listInvoice.get(i));
					sodonhangcnduoc++;
				}
			}
		...
		try {
			...
			for (int i = 0; i < listInvoice.size(); i++) {
				if (listInvoice.get(i).isChonphieu()) {
					if (listInvoice.get(i).getCar() != null) {
						listInvoice.get(i).setSoxe(listInvoice.get(i).getCar().getLicense_plate());
					} else {
						listInvoice.get(i).setSoxe("");
					}
	...
	public void taoHoaDonChiNhanh() {
		...
		try {
			...
			for (int i = 0; i < listInvoice.size(); i++) {
				if (listInvoice.get(i).isChonphieu()) {
					if (listInvoice.get(i).isPhieudacopy()) {
						status = false;
						break;
					} else {
						if (carChoose == null) {
							carChoose = listInvoice.get(i).getCar();
						} else {
							if (!carChoose.equals(listInvoice.get(i).getCar())) {
								status = false;
								break;
							}
						}
						if (listInvoice.get(i).getLookup_code() == null
								|| "".equals(listInvoice.get(i).getLookup_code())) {
							status = false;
							break;
						}
						invoiceChoose.add(listInvoice.get(i));
				...
				if (allowDelete(wrapInvoiceDetailSelect.getInvoice_detail().getInvoice().getInvoice_date(),
					...
					if (detail != null) {
						if (detail.getInvoice() != null
								&& allowDelete(detail.getInvoice().getInvoice_date(), detail.getInvoice()
										.getCreated_by())) {
	...
	public void deleteInvoice() {
		...
		try {
			current.executeScript("PF('tablect').clearFilters();");
			if (invoiceSelect != null) {
				if (allowDelete(invoiceSelect.getInvoice_date(), invoiceSelect.getCreated_by())) {
		...
		try {
			if (invoiceCrud != null && invoiceCrud.getId() != 0) {
				if (wrapInvoiceDetailCrud != null) {
					...
					Product product = invoiceDetail.getProduct();
					if (product != null) {
						WrapPMInvoiceDetailReqInfo wrap = new WrapPMInvoiceDetailReqInfo(invoiceDetail.clone(), account
								.getMember().getName());
						Message message = new Message();
						HeThong ht = heThongService.findById(1);
	...
	public void xacNhanThanhToan(Invoice invoice) {
		try {
			int result = invoiceService.updatePayment(invoice.getId(), getAccount().getUserName(), invoice.isPayment());
			if (result == 1) {
				InvoiceReqInfo invoiceReqInfo = new InvoiceReqInfo();
		...
		try {
			...
			if (part != null) {
				...
				if (hoaDonOnlines.size() != 0) {
					Map<String, Map<String, Map<String, Map<Double, List<DuLieuHoaDonOnline>>>>> hds = hoaDonOnlines
							.stream().collect(
									Collectors.groupingBy(
											DuLieuHoaDonOnline::getMadonhang,
											Collectors.groupingBy(
					...
					for (String madh : hds.keySet()) {
						Map<String, Map<String, Map<Double, List<DuLieuHoaDonOnline>>>> hds1 = hds.get(madh);
						...
						if (orderLixCrud == null) {
							...
							if (listWrapOrderDetailLixReqInfo.size() > 0) {
								...
								int code = processLogicInvoiceService.createInvoiceByExtensionOrder(data, listIdResult,
										messages, hdssub.get(0).getSoxe(), ht.isKiemton());
								if (code == 0) {
									success();
								} else {
