...
public class YeuCauKiemTraHangService {
    ...
    public int search(String json, List<YeuCauKiemTraHang> list) {
        ...
        try {
            ...
            List<Predicate> allPredicates = new ArrayList<>(); // Danh sách chứa tất cả các điều kiện

            // --- Xử lý điều kiện ngày ---
            Date fromDate = ToolTimeCustomer.convertStringToDate(Objects.toString(hFromDate.getValue(), null), "dd/MM/yyyy");
            Date toDate = ToolTimeCustomer.convertStringToDate(Objects.toString(hToDate.getValue(), null), "dd/MM/yyyy");

            if (fromDate != null) {
                allPredicates.add(cb.greaterThanOrEqualTo(root_.get("requestDate"), fromDate));
            }

            if (toDate != null) {
                allPredicates.add(cb.lessThanOrEqualTo(root_.get("requestDate"), toDate));
            }

            // --- Xử lý điều kiện tìm kiếm chuỗi (stextStr) ---
            List<Predicate> predicatesStr = new ArrayList<>();
            String stextStr = Objects.toString(hstextStr.getValue());

            if (hstextStr.getValue() != null && !stextStr.trim().isEmpty()) {
                // Điều kiện cho requestCode
                Predicate predicateRequestCode = cb.like(root_.get("requestCode"), "%" + stextStr + "%");
                predicatesStr.add(predicateRequestCode);

                // Điều kiện cho createdBy
                Predicate predicateCreatedBy = cb.like(root_.get("createdBy"), "%" + stextStr + "%");
                predicatesStr.add(predicateCreatedBy);

                // Điều kiện tìm kiếm trong chi tiết (YeuCauKiemTraHangDetail)
                Subquery<YeuCauKiemTraHangDetail> subqueryDetail = cq.subquery(YeuCauKiemTraHangDetail.class);
                Root<YeuCauKiemTraHangDetail> subrootDetail = subqueryDetail.from(YeuCauKiemTraHangDetail.class);

                List<Predicate> subPredicatesList = new LinkedList<>();
                Predicate predicatePdCode = cb.like(subrootDetail.get("product").get("product_code"), "%" + stextStr + "%");
                subPredicatesList.add(predicatePdCode);
                Predicate predicatePdName = cb.like(subrootDetail.get("product").get("product_name"), "%" + stextStr + "%");
                subPredicatesList.add(predicatePdName);

                subqueryDetail.select(subrootDetail.get("yeuCauKiemTraHang"))
                              .where(cb.or(subPredicatesList.toArray(new Predicate[0])));

                Predicate predicateSub = cb.equal(root_, cb.any(subqueryDetail));
                predicatesStr.add(predicateSub); // Thêm điều kiện truy cập chi tiết
            }

            // --- Kết hợp tất cả các điều kiện ---
            if (!predicatesStr.isEmpty()) {
                // Nếu có điều kiện tìm kiếm chuỗi, kết hợp với các điều kiện ngày bằng AND
                // và các điều kiện chuỗi bằng OR
                allPredicates.add(cb.or(predicatesStr.toArray(new Predicate[0])));
            }

            // Áp dụng tất cả các Predicate đã thu thập vào mệnh đề WHERE
            if (!allPredicates.isEmpty()) {
                cq.where(cb.and(allPredicates.toArray(new Predicate[0])));
            }

            // --- Sắp xếp kết quả ---
            cq.orderBy(cb.desc(root_.get("requestDate")), cb.desc(root_.get("requestCode")));
            ...
            // Logger.getLogger(YourServiceClass.class.getName()).log(Level.SEVERE, "Error in search method", e);
        }
        return res;
    }
