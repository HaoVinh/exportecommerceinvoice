...
public class FormListDataService {
	 ...
	 */
	public static FormListData[] dsform(String param) {
		try {
			String link="?cm=dsform&dt=" + param;
			String data = process(link);
			DataResponseAPI ketqua = gson.fromJson(data, DataResponseAPI.class);
			FormListData[] formListDatas = gson.fromJson(ketqua.getDt(), FormListData[].class);
			return formListDatas;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
