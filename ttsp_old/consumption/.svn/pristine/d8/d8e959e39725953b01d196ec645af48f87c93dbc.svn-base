		...
		try {
			...
			if (tinhlaidanhsach) {
				...
				if (iEInvoiceDetailCrud != null && iEInvoiceDetailCrud.getProduct() != null) {
					...
					iEInvoiceDetailCrud.setQuantity((double) MyMath.roundCustom(iEInvoiceDetailCrud.getQuantity_export(), 2));
					// tính toán số tiền ngoại tệ
					double totalForeignAmount = BigDecimal.valueOf(quantity)
							.multiply(BigDecimal.valueOf(iEInvoiceDetailCrud.getForeign_unit_price())).doubleValue();
...
//				quantityBatchInvCurr = batchService.getQuantityRemaining(iEInvoiceDetailCrud.getProduct().getId());
	...
	String[] maspincartons = { "L4001", "L4002", "L4003", "L4004", "L4101", "L4102", "L4103", "L4104", "L4201",
			"L4202", "L4203", "L4301", "L4302", "L4303" };
	...
	private void exportCommercialInvoice() {
		...
		try {
			...
			String lastUnit = "CTNS";
			int dvbao = 0, dvtui = 0, dvkhac = 0;
			String order_no = null;
			boolean unit_p_PSC = false;
			for (IECommercialInvoiceReportInfo p : list) {

				boolean status = false;
				for (int i = 0; i < maspincartons.length; i++) {
					if (maspincartons[i].equals(p.getProduct_code())) {
						status = true;
						break;
					}
				}
				if (status) {
					...
					if ("XÔ".equals(p.getUnit()) || "BAO".equals(p.getUnit())) {
						p.setUnit("PKGS");
						lastUnit = "PKGS";
						dvbao++;
					} else if ("TÚI".equals(p.getUnit())) {
				...
				if (order_no == null || "".equals(order_no))
					order_no = p.getOrderno();
			}
			boolean donvikhacnhau = false;
			if ((dvbao != 0 && dvtui == 0 && dvkhac == 0) || (dvtui != 0 && dvbao == 0 && dvkhac == 0)
					|| (dvkhac != 0 && dvbao == 0 && dvtui == 0)) {
			} else {
				donvikhacnhau = true;
			}
			if (donvikhacnhau)
				lastUnit = "PKGS";
			importParam.put("list_data", list);
			importParam.put("last_unit", lastUnit);
			importParam.put("invoice_date",
					ToolTimeCustomer.convertDateToString(iEInvoiceCrud.getInvoice_date(), "dd/MM/yyyy"));
			...
			for (IECommercialInvoiceReportInfo ie : list) {
				...
				if (unit_p_PSC) {
					ie.setUnit_p("PCS");
					importParam.put("unit_p", "PCS");
				} else {
					importParam.put("unit_p", "BOTTLES");
				}
	...
	private void exportPackingList() {
		...
		try {
			...
			String order_no = null;
			boolean unit_p_PSC = false;
			for (IEPackingListReport p : list) {
				boolean status = false;
				for (int i = 0; i < maspincartons.length; i++) {
					if (maspincartons[i].equals(p.getProduct_code())) {
						status = true;
	...
	private void exportPackingListOut() {
		...
		try {
			...
			importParam.put("invoice_date",
					ToolTimeCustomer.convertDateToString(iEInvoiceFirst.getInvoice_date(), "dd/MM/yyyy"));
			importParam.put("shipped_per", iEInvoiceCrud.getShipped_per());
			String a = iEInvoiceCrud.getTerm_of_delivery()
					+ (iEInvoiceCrud.getHarbor_category() != null ? " "
							+ iEInvoiceCrud.getHarbor_category().getHarbor_name() : "");
			// importParam.put("term_of_delivery",
			// iEInvoiceCrud.getTerm_of_delivery()
		...
		try {
			...
			if (datas.size() > 0) {
				...
				Object[] title = { "IdPhieuXuat", "SoCT", "Ngay", "MaKH", "TenKH", "SoXe", "HTThanhToan", "MaNX",
						"ChiChu", "LySoXuat", "BocXep", "SoHopDong", "HTLenHang", "NgayTauChay","NgayTauDen","DueDate", "DaThanhToan",
						"DaGiaoHang", "TyGia", "BillNo", "MaToKhai", "CangXuat", "DonViTien", "Reference_no",
						"shipped_per", "term_of_delivery", "shipping_mark", "Phi", "TenTaiXe", "DiemGiaoHang",
						"DiemNhanHang", "IDChiTiet", "MaSP", "TenSp", "SL", "SoTienNT", "DonGiaNT", "SoTienNTXK",
						"DonGia", "SoTien", "MaDH", "MaCont", "KichThuoc", "SoCont", "SoSeal", "MaLH", "ND", "SoLuong(Thung)","SoLuong(KG)" };
				results.add(title);
				for (Object[] it : datas) {
					results.add(it);
				}
