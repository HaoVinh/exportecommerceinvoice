...
public class FormUpGoodsService implements IFormUpGoodsService {
	...
	@Override@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public int selectByCode(String code, FormUpGoodsReqInfo t) {
		int res = -1;
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<FormUpGoods> cq = cb.createQuery(FormUpGoods.class);
	...
	@Override@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public int complete(String text, List<FormUpGoods> list) {
		int res = -1;
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<FormUpGoods> cq = cb.createQuery(FormUpGoods.class);
			...
			dis.getExpressions().add(cb.like(root.get("old_code"), "%" + text + "%"));
			cq.select(root).where(dis);
			TypedQuery<FormUpGoods> query = em.createQuery(cq);
			list.addAll(query.getResultList());
			res = 0;
		} catch (Exception e) {
			logger.error("FormUpGoodsService.complete:" + e.getMessage(), e);
		}
		return res;
	}
