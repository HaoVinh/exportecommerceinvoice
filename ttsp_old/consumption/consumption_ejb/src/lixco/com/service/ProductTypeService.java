...
public class ProductTypeService implements IProductTypeService {
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public int selectAllOrderByName(List<ProductType> list) {
		int res = -1;
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<ProductType> cq = cb.createQuery(ProductType.class);
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public int complete(String text, List<ProductType> list) {
		int res = -1;
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<ProductType> cq = cb.createQuery(ProductType.class);
			...
			cq.select(cb.construct(ProductType.class, root.get("id"), root.get("code"), root.get("name"))).where(dis);
			TypedQuery<ProductType> query = em.createQuery(cq);
			list.addAll(query.getResultList());
			res = 0;
		} catch (Exception e) {
