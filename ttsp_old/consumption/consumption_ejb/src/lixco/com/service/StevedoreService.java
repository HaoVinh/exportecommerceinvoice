...
public class StevedoreService implements IStevedoreService{
	...
	@Override
	public int selectByCode(String code, StevedoreReqInfo t) {
		int res=-1;
		try{
			CriteriaBuilder cb=em.getCriteriaBuilder();
			CriteriaQuery<Stevedore> cq=cb.createQuery(Stevedore.class);
			...
			TypedQuery<Stevedore> query=em.createQuery(cq);
			t.setStevedore(query.getSingleResult());
			res=0;
		}catch(Exception e){
			logger.error("StevedoreService.selectByCode:"+e.getMessage(),e);
		}
		return res;
	}
	@Override
	public int complete(String text, List<Stevedore> list) {
		int res = -1;
		try {
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<Stevedore> cq = cb.createQuery(Stevedore.class);
			...
			dis.getExpressions().add(cb.equal(root.get("old_code"), text));
			...
			logger.error("StevedoreService.complete:" + e.getMessage(), e);
		}
		return res;
	}
