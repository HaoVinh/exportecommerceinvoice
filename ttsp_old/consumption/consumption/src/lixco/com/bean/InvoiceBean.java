...
public class InvoiceBean extends AbstractBean {
	...
	// if (code == 0) {
	// current.executeScript("swaldesigntimer('Thành công','Hóa đơn đã hoàn thành việc ghi nhận doanh thu','success',2000)");
	// // load lại invoice
	...
	// }
	// } else {
	// current.executeScript("swaldesigntimer('Không thành công','Hóa đơn chưa được lưu','warning',2000)");
	// }
	// } catch (Exception e) {
	...
	// try {
	// if (invoiceCrud != null && invoiceCrud.getId() != 0) {
	// current.executeScript("swalfunction('Thông báo', 'Hóa đơn này sẽ không được ghi nhận doanh thu nhưng vẫn tính tồn kho, bạn có muốn cập nhật không?','info','destroyInvoice()');");
	// } else {
	...
	// message);
	// if (code == 0) {
	// current.executeScript("swaldesigntimer2('Thành công','Hóa đơn đã hủy việc ghi nhận doanh thu','success',2000)");
	// // load lại invoice
	// InvoiceReqInfo invoiceReqInfo = new InvoiceReqInfo();
	// invoiceService.selectById(invoiceCrud.getId(), invoiceReqInfo);
	// int index = listInvoice.indexOf(invoiceCrud);
	...
	public void capnhatiddonhang2() {
		...
		for (int i = 0; i < listInvoice.size(); i++) {
			...
			if (iv.getOrder_voucher() != null && !"".equals(iv.getOrder_voucher()) && iv.getIdorderlix() == 0) {
				List<OrderLix> orderLixs = orderLixService.selectByOrderVoucher(iv.getOrder_voucher(), iv.getCustomer()
						.getId());
	...
	public void inPhieuVanChuyenHangHoa() {
		...
		try {
			...
			Map<String, List<Invoice>> datagroupsphieus = invoiceChoose.stream().collect(
					Collectors.groupingBy(p -> p.getSoxe(), Collectors.toList()));
			...
			for (String key_0 : datagroupsphieus.keySet()) {
				...
				if (key_0 != null && invoiceChoose1.size() != 0) {
					...
					Map<Customer, List<Invoice>> datagroups1 = invoiceChoose1.stream().collect(
							Collectors.groupingBy(p -> p.getCustomer(), Collectors.toList()));
					...
					for (Customer key : datagroups1.keySet()) {
						...
						for (int i = 0; i < size; i++) {
							...
							for (int j = 0; j < invoiceDetails.size(); j++) {
								double kl = (invoiceDetails.get(j).getQuantity() * invoiceDetails.get(j).getProduct()
										.getFactor());
								khoiluong += kl;
							}
						}
			...
			if (status == false) {
				...
				if (invoiceChoose.size() != 0) {
					...
					if (daCopy) {
						...
						Map<Product, List<InvoiceDetail>> datagroupschitiets = invoiceDetails.stream().collect(
						...
						Call call = trong.lixco.com.api.CallAPI.getInstance(token).getMethodPost(
								path + "/api/data/taohoadon", gson.toJson(dataAPI));
	...
	public void acceptDelInvoiceDetail() {
		...
		try {
			if (allowDelete(wrapInvoiceDetailSelect.getInvoice_detail().getInvoice().getInvoice_date(),
				...
				WrapDelInvoiceDetailReqInfo t1 = new WrapDelInvoiceDetailReqInfo(account.getMember().getId(), account
						.getMember().getName(), wrapInvoiceDetailSelect.getInvoice_detail().getId());
				List<String> list = new ArrayList<>();
				invoiceService.prepareDataDelInvoiceDetail(wrapInvoiceDetailSelect.getInvoice_detail().getId(), list);
				Message message = new Message();
				int code = processLogicInvoiceService
						.deleteInvoiceDetailMaster(t1, message, getAccount().getUserName());
				if (code == 0) {
	...
	public void deleteInvoiceFromBean(long invoice_id) {
		try {
			...
			if (allowDelete(invoice.getInvoice_date(), invoice.getCreated_by())) {
				...
				WrapInvoiceDelInfo t1 = new WrapInvoiceDelInfo(invoice_id, account.getMember().getId(), account
						.getMember().getName());
				...
				executeScript("swaldesigntimer('Cảnh báo!', 'Tài khoản này không có quyền thực hiện hoặc tháng đã khoá!','warning',2000);");
			}
		} catch (Exception e) {
		...
		try {
			...
			if (listInvoiceDetail.size() > 0) {
				...
				for (InvoiceDetail it : listInvoiceDetail) {
					String cus = it.getInvoice().getCustomer() != null ? it.getInvoice().getCustomer()
							.getCustomer_code() : "";
					String wah = it.getInvoice().getWarehouse() != null ? it.getInvoice().getWarehouse().getCode() : "";
					String ie = it.getInvoice().getIe_categories() != null ? it.getInvoice().getIe_categories()
							.getCode() : "";
					String ct = it.getInvoice().getContract() != null ? it.getInvoice().getContract()
							.getContract_code() : "";
		...
		try {
			...
			if (datas.size() > 0) {
				...
				Object[] title = { "Mã KH xuất", "Số CT", "Ngày", "Mã KH", "Mã kho", "Mã XN", "Số HĐ", "Mã SP",
						...
						"Mã SP chính", "Quy đổi SP chính", "Mã SP KM", "Quy đổi SP KM", "Thủ Kho", "Ghi chú CTKM",
						"Số lượng pallet", "Số HĐ thương mại", "Nhãn hàng", "Ngày HĐ gốc" };
				results.add(title);
				String chinhanh = getDatabase();
				String nameCN = "";
				String makhachhang = "";
	...
	public void acceptDelInvoice() {
		...
		try {
			...
			if (iv != null) {
				if (allowDelete(iv.getInvoice_date(), iv.getCreated_by())) {
					WrapInvoiceDelInfo t1 = new WrapInvoiceDelInfo(iv.getId(), account.getMember().getId(), account
							.getMember().getName());
					// chuẩn bị dữ liệu để delete
					Message message = new Message();
					int code = processLogicInvoiceService.deleteInvoiceMaster(t1, message, getAccount().getUserName());
						...
						if ("CKDH".equals(product.getProduct_code())) {
							...
							if (caidgkm) {
								...
								json.addProperty("date", ToolTimeCustomer.convertDateToString(
										invoiceCrud.getInvoice_date(), "dd/MM/yyyy"));
		...
		try {
			if (invoiceCrud != null) {
				...
				if (ieCategories != null && customer != null && invoiceDate != null && warehouse != null
						...
						if (invoiceCrud.getId() == 0) {
							...
							if (invoiceOld != null
								...
								if (invoiceService.updateSelect(invoiceCrud.clone(), getAccount().getUserName()) > 0) {
									...
									current.executeScript("swaldesigntimer('Không thành công!', 'Lưu Không thành công!','warning',2000);");
								}
							} else {
								if (!allowUser(invoiceOld.getCreated_by())) {
									...
									if (invoiceOld != null && allowUpdate(null, invoiceOld.getCreated_by())) {
										...
										if (invoiceService.updateSelect(invoiceOld, getAccount().getUserName()) > 0) {
											...
											current.executeScript("swaldesigntimer('Cảnh báo!', 'Xảy ra lỗi không cập nhật được.','error',2000);");
										}
					...
					current.executeScript("swaldesigntimer('Cảnh báo!', 'Thông tin hóa đơn không đầy đủ, điền đầy đủ thông tin chứa(*)','warning',2000);");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void chuyenHDDTDS(boolean phieuxuatkho) {
		...
		if (invoiceChoose.size() > 20 && (hoadonTMDT == false || invoiceChoose.size() > 300)) {
			current.executeScript("swaldesigntimer2('Cảnh báo','Không chuyển được quá 20 hóa đơn thường hoặc 300 hóa đơn TMĐT.','warning',4000)");
		} else {
			if (invoiceChoose.size() != 0) {
				for (int i = 0; i < invoiceChoose.size(); i++) {
		...
		try {
			if ("LIXCO@2023HD".equals(passhoadonjson)) {
				...
				if (capnhatphieu) {
							...
							if (kqinsert != null) {
								if (kqinsert.getCode() == 200) {
									...
									if (result.contains("Duplicate entry")) {
										current.executeScript("swaldesigntimer2('Không thành công','Phiếu đã nạp lên hóa đơn điện tử','warning',2000)");
									} else {
										current.executeScript("swaldesigntimer2('Không thành công','Xảy ra lỗi, nhấn F12 để xem chi tiết','error',2000)");
										current.executeScript("console.log('" + result + "')");
									}
					...
					current.executeScript("swaldesigntimer2('Cảnh báo','Không cập nhật được phiếu, nhấn F12 để xem chi tiết','warning',2000)");
				}
			} else {
				current.executeScript("swaldesigntimer2('Cảnh báo','Không đúng mật khẩu chuyển hóa đơn.','warning',2000)");
			}
					...
					if (capnhatphieu) {
						...
						if (!phieuxuatkho) {
							thongbao = InvoiceToJson.toJsonHoaDon(invoiceLoad, eInvoiceService, invoiceService,
									invoiceDetailService, ieInvoiceService, lamtron, getDatabase(), getAccount()
											.getUserName(), productService);
						} else {
						...
						if (thongbao.isLoi() == false) {
							...
							try {
								System.out.println("Chuyen hoa don-------" + invoiceLoad.getId() + "\t refID: "
								...
								System.out.println("\tDone" + invoiceLoad.getId() + "\t refID: "
										+ invoiceLoad.getRefId());
							} catch (Exception e) {
								e.printStackTrace();
							}
			...
			if (part != null) {
				...
				if (hoaDonOnlines.size() != 0) {
					Map<String, Map<String, Map<String, Map<String, List<DuLieuHoaDonOnline>>>>> hds = hoaDonOnlines
							.stream().collect(
									Collectors.groupingBy(DuLieuHoaDonOnline::getMadonhang, Collectors.groupingBy(
											DuLieuHoaDonOnline::getMakh,
					...
					for (String madh : hds.keySet()) {
						...
						for (String makh : hds1.keySet()) {
							...
							if (customer == null) {
								...
								JsonObject json = new JsonObject();
								json.addProperty("date", ToolTimeCustomer.convertDateToString(new Date(), "dd/MM/yyyy"));
													...
													if ("P".equalsIgnoreCase(loainhapxuat)) {
														...
														if (dongiamoi != null) {
															...
															if (idPriceSub != 0) {
																...
																if (t1.getPricing_program_detail() != null) {
																	if (t1.getPricing_program_detail().getUnit_price() != 0) {
																		detail.setUnit_price(t1
																				.getPricing_program_detail()
																				.getUnit_price());
																		statusPrice = false;
																	}
																}
															...
															dulieukhongnapduoc.add("Sản phẩm: "
																	+ detail.getProduct().getProduct_code()
											...
											if (product != null) {
												...
												if ((boolean) dutk[0]) {
													...
													dulieukhongnapduoc.add("Sản phẩm: "
															...
															+ " không đủ tồn kho (xuất: "
															+ MyUtilEJB.dinhdangso(detail.getQuantity())
															+ " -> tồn kho: " + MyUtilEJB.dinhdangso((double) dutk[1])
															+ ")");
												}
											} else {
												dulieukhongnapduoc.add("Không tìm thấy mã sản phẩm: " + masp);
								...
								if (invoiceDetailDonGia0MaPs.size() != 0) {
									...
									invoiceKM.setExported(true);
									invoiceKM.setCarrier(carrier);
									double totalevent = invoiceDetailDonGia0MaPs.stream()
			...
			if (part != null) {
				...
				while (rows.hasNext()) {
					...
					while (cells.hasNext()) {
						...
						case 6:// nhom hoa don
							try {
								nhomtheohd = (int) Double.parseDouble(Objects.toString(MyUtilExcel.getCellValue(cell),
										"0"));
							} catch (Exception e) {
							}
							break;
					...
					if (masp != null && makh != null && !"".equals(makh)) {
						DuLieuHoaDonVSIP hd = new DuLieuHoaDonVSIP(makh, masp, soluong, malohang, ghichuxuat,
								nhomtheohd, slpallet);
						hoaDonVSIPs.add(hd);
					}
				}
				if (hoaDonVSIPs.size() != 0) {
					Map<String, Map<Integer, List<DuLieuHoaDonVSIP>>> hds = hoaDonVSIPs.stream().collect(
							Collectors.groupingBy(DuLieuHoaDonVSIP::getMakh,
					...
					if (invoiceCrud != null) {
						while (rows.hasNext()) {
							...
							while (cells.hasNext()) {
								...
								case 2:// dongia
									try {
										dongia = Double.parseDouble(Objects.toString(MyUtilExcel.getCellValue(cell),
												"0"));
									} catch (Exception e) {
									}
									break;
								...
								case 3:// soluong
									try {
										soluong = Double.parseDouble(Objects.toString(MyUtilExcel.getCellValue(cell),
												"0"));
									} catch (Exception e) {
									}
									break;
								...
								case 4:// thanhtien
									try {
										thanhtien = Double.parseDouble(Objects.toString(MyUtilExcel.getCellValue(cell),
												"0"));
									} catch (Exception e) {
									}
									break;
