...
@Path("data/datasanpham")
public class SanPhamServlet2 {
	@Context
	...
	@Produces("application/json; charset=utf-8")
	public Response thongTinSanPham() throws IOException,
			ServletException {
		StringBuilder result = new StringBuilder();
		ResponseBuilder responseBuilder = Response.status(Response.Status.BAD_REQUEST);
		String dsspStr = request.getHeader("dssanpham");
		List<String>masps=Arrays.asList(dsspStr.split(";"));
		try {
			List<SanPhamData2> sanPhamDatas = productService.selectByCodes(masps);
			if (sanPhamDatas.size() != 0) {
				JsonObject jsonObject = new JsonObject();
				jsonObject.add("data", JsonParserUtil.getGson().toJsonTree(sanPhamDatas));
				responseBuilder.status(Response.Status.OK);
	...
	@GET
	@Path("thongtinsanpham2")
	@Produces("application/json; charset=utf-8")
	public Response thongTinSanPham2() throws IOException,
			ServletException {
		StringBuilder result = new StringBuilder();
		ResponseBuilder responseBuilder = Response.status(Response.Status.BAD_REQUEST);
		String dsspStr = request.getHeader("dssanpham");
		String loaima = request.getHeader("loaima");
		List<String>masps=Arrays.asList(dsspStr.split(";"));
		try {
			List<SanPhamData2> sanPhamDatas = productService.selectByCodeLixAndULs(masps,loaima);
			if (sanPhamDatas.size() != 0) {
				JsonObject jsonObject = new JsonObject();
				jsonObject.add("data", JsonParserUtil.getGson().toJsonTree(sanPhamDatas));
				responseBuilder.status(Response.Status.OK);
