...
import lixco.com.service.InvoiceDeleteService;
...
import trong.lixco.com.apitaikhoan.MemberData;
...
import trong.lixco.com.info.DuLieuHoaDonOnline2;
...
import trong.lixco.com.util.CallAPIMBNapVaoMN;
...
public class InvoiceBean extends AbstractBean {
	...
	private final String MANX_ONLINE[] = { "$", "&" };
	@Inject
	...
	private InvoiceDTO3 invoiceSelect;
	...
	@Getter
	@Setter
	private List<Customer> customerSearchs;
	@Getter
	@Setter
	private List<CustomerTypes> customerTypesSearchs;
	@Getter
	@Setter
	private List<CustomerChannel> customerChannelSearchs;
	@Getter
	@Setter
	...
	@Getter
	@Setter
	private List<IECategories> ieCategoriesSearchs;
	@Getter
	@Setter
	...
	protected void initItem() {
		try {
			...
			CotHienThi cothienthi = cotHienThiService.find(TENBANG, account.getUserName());
			if (cothienthi != null) {
			...
			paymentSearch = -1;
			fromDateSearch = ToolTimeCustomer.plusDayNow(-3);
		...
		try {
			if (invoiceCrud != null) {
				...
				if (obj != null) {
					...
					if (invoiceCrud.getInvoice_date() != null) {
						// cập nhật chương trình đơn giá cho đơn hàng
						JsonObject json = new JsonObject();
						json.addProperty("date",
								ToolTimeCustomer.convertDateToString(invoiceCrud.getInvoice_date(), "dd/MM/yyyy"));
	...
	public void viewInventory() {
		try {
			if (invoiceCrud != null) {
				...
				invoiceService.getListInventory(listProductId, invoiceCrud.getInvoice_date(), list);
				if (list.size() > 0) {
	...
	// if (index != -1) {
	// listInvoice.set(index, invoiceReqInfo.getInvoice());
	// }
		...
		try {
			if (listInvoice != null && listInvoice.size() > 0) {
				...
				for (int i = 0; i < listInvoice.size(); i++) {
					if (listInvoice.get(i).getId() == invoiceCrud.getId()) {
						index = i;
						break;
					}
				}
	...
	public void exportLenhDieuDongLever() {
		...
		try {
			...
			int day = ToolTimeCustomer.getDayM(invoiceCrud.getInvoice_date());
			int month = ToolTimeCustomer.getMonthM(invoiceCrud.getInvoice_date());
			int year = ToolTimeCustomer.getYearM(invoiceCrud.getInvoice_date());
	...
	public void exportHoaDon() {
		...
		try {
			String reportPath = FacesContext.getCurrentInstance().getExternalContext()
					.getRealPath("/resources/reports/invoice/hoadon.jasper");
			Map<String, Object> importParam = new HashMap<String, Object>();
			Locale locale = new Locale("vi", "VI");
			importParam.put(JRParameter.REPORT_LOCALE, locale);
			List<ParamReportDetail> listConfig = paramReportDetailService.findByParamReportName("thongtinchung");
			...
			ThongBao thongbao = InvoiceToJson.toJsonHoaDon(invoiceCrud, eInvoiceService, invoiceService,
					invoiceDetailService, ieInvoiceService, lamtron, getDatabase(), getAccount().getUserName(),
					productService);
			if (thongbao.isLoi() == false) {
				List<EInvoiceV3> eInvoiceDatas = JsonParserUtil.getGson().fromJson(thongbao.getDulieu(),
						new TypeToken<List<EInvoiceV3>>() {
						}.getType());
				if (eInvoiceDatas != null && eInvoiceDatas.size() != 0) {
					EInvoiceV3 eInvoiceData = eInvoiceDatas.get(0);
					importParam.put("ngaychungtu", invoiceCrud.getInvoice_date());
					importParam.put("tendonvi", eInvoiceData.getAccountObjectName());
					importParam.put("diachi", eInvoiceData.getAccountObjectAddress());
					importParam.put("masothue", eInvoiceData.getAccountObjectTaxCode());
					String solenhdieudong = "/" + (invoiceCrud.getInvoice_date().getYear() + 1900) + "/LĐĐ";
					importParam.put("ldd", solenhdieudong);
					importParam.put("thanhtoan", eInvoiceData.getPaymentMethod());
					importParam.put("taikhoan", "");

					importParam.put("thuesuatgtgt", eInvoiceData.getVATRate());
					importParam.put("congtienhang", eInvoiceData.getTotalAmountWithoutVAT());
					importParam.put("thuevat", eInvoiceData.getTotalVATAmount());
					importParam.put("tongcong", eInvoiceData.getTotalAmountWithVAT());

					// Tong tien viet bang chu
					String tiendocbangchu = "";
					try {
						tiendocbangchu = ConvertNumberToText.docSo(Math.abs(eInvoiceData.getTotalAmountWithVAT()),
								"ĐỒNG");
					} catch (Exception e) {
						// TODO: handle exception
					}
					importParam.put("tongtienbangchu", tiendocbangchu);

					importParam.put("list_data", eInvoiceData.getInvoiceDetails());
					JasperPrint jasperPrint = JasperFillManager.fillReport(reportPath, importParam,
							new JRBeanCollectionDataSource(eInvoiceData.getInvoiceDetails()));
					byte[] data = JasperExportManager.exportReportToPdf(jasperPrint);
					String ba = Base64.getEncoder().encodeToString(data);
					current.executeScript("utility.printPDF('" + ba + "')");
				}
	...
	@Getter
	@Setter
	private boolean chuachuyenmisa;
	@Getter
	@Setter
	private int giohanHDtruyvan;
	...
	public void search() {
		try {
			...
			invoiceService.search(JsonParserUtil.getGson().toJson(json), listInvoice, customerChannelSearchs,
					customerTypesSearchs, chuachuyenmisa, giohanHDtruyvan, ieCategoriesSearchs, customerSearchs);
			long endTime = System.currentTimeMillis();
			long duration = endTime - startTime;
			System.out.println("Thời gian thực thi: " + duration + " mili giây");
	...
	public void capnhatiddonhang() {
		...
		for (int i = 0; i < listInvoice.size(); i++) {
			Invoice iv = invoiceService.findById(listInvoice.get(i).getId());
			if (iv.getIdorderlix() == 0) {
				List<InvoiceDetail> listInvoiceDetail = new ArrayList<>();
				invoiceDetailService.selectByInvoice(listInvoice.get(i).getId(), listInvoiceDetail);
				...
				if (idchitietdonhang != 0) {
					...
					if (od != null && od.getInvoice().getIdorderlix() != 0) {
						iv.setOrder_voucher(od.getInvoice().getOrder_voucher());
						iv.setIdorderlix(od.getInvoice().getIdorderlix());
						invoiceService.updateDonHang(iv);
						sodonhangcnduoc++;
					}
				}
			}
	...
	public void capnhatiddonhang2() {
		...
		for (int i = 0; i < listInvoice.size(); i++) {
			Invoice iv = invoiceService.findById(listInvoice.get(i).getId());
			if (iv.getOrder_voucher() != null && !"".equals(iv.getOrder_voucher()) && iv.getIdorderlix() == 0) {
				List<OrderLix> orderLixs = orderLixService.selectByOrderVoucher(iv.getOrder_voucher(),
						iv.getCustomer().getId());

				if (orderLixs.size() == 1) {
					iv.setOrder_voucher(orderLixs.get(0).getVoucher_code());
					iv.setIdorderlix(orderLixs.get(0).getId());
					invoiceService.updateDonHang(iv);
					sodonhangcnduoc++;
				}
			}
	...
	public void capnhatsodonhang() {
		...
		for (int i = 0; i < listInvoice.size(); i++) {
			Invoice iv = invoiceService.findById(listInvoice.get(i).getId());
			if (iv.getIdorderlix() != 0) {
				OrderLix orderLix = orderLixService.findById(iv.getIdorderlix());
				if (orderLix != null) {
					iv.setOrder_voucher(orderLix.getVoucher_code());
					iv.setIdorderlix(orderLix.getId());
					invoiceService.updateDonHang(iv);
					sodonhangcnduoc++;
				}
			}
	...
	public void inPhieuVanChuyenHangHoa() {
		...
		try {
			...
			// Car carChoose = null;
			for (int i = 0; i < listInvoice.size(); i++) {
				if (listInvoice.get(i).isChonphieu()) {
					Invoice iv = invoiceService.findById(listInvoice.get(i).getId());
					if (iv.getCar() != null) {
						iv.setSoxe(iv.getCar().getLicense_plate());
					} else {
						iv.setSoxe("");
					}
					invoiceChoose.add(iv);
				}
			}
		...
		try {
				...
				if (listInvoice.get(i).isChonphieu()) {
					...
					if (iv.isPhieudacopy()) {
						...
						if (carChoose == null) {
							...
							if (!carChoose.equals(iv.getCar())) {
								status = false;
								break;
							}
						}
						if (iv.getLookup_code() == null || "".equals(iv.getLookup_code())) {
							status = false;
							break;
						}
						invoiceChoose.add(iv);
			...
			if (status == false) {
				...
				if (invoiceChoose.size() != 0) {
					...
					// Kiem tra da copy chua
					boolean daCopy = invoiceService.checkIsCopy(idinvoices);
					if (daCopy) {
						canhbao("Có phiếu đã copy, vui lòng tải lại danh sách và chọn lại.");
					} else {
						...
						invoiceDetailDTOs = invoiceDetailDTOs.stream()
								.sorted(Comparator.comparingLong(InvoiceDetailDTO::getIdtc))
	...
	public void chonHetPhieu() {
		...
		if (listInvoice != null) {
			for (int i = 0; i < listInvoice.size(); i++) {
				listInvoice.get(i).setChonphieu(chonhet);
			}
			if (chonhet)
				sophieuchon = listInvoice.size();
		}
	}
	...
	public void showDialogInvoiceDetail() {
		...
		try {
			if (invoiceCrud != null && invoiceCrud.getId() != 0) {
				...
				invoiceDetail.setInvoice(invoiceCrud);
	...
	public List<PricingProgram> completePricingProgram(String text) {
		try {
			...
			if (invoiceCrud.getCustomer() != null && invoiceCrud.getCustomer().getId() != 0
					&& invoiceCrud.getInvoice_date() != null) {
				JsonObject json = new JsonObject();
				json.addProperty("date",
						ToolTimeCustomer.convertDateToString(invoiceCrud.getInvoice_date(), "dd/MM/yyyy"));
	...
	private void sumInvoice() {
		try {
			if (listWrapInvoiceDetail != null) {
				double totalevent = listWrapInvoiceDetail.stream().mapToDouble(f -> f.getInvoice_detail().getTotal())
						.sum();
				invoiceCrud.setTongtien(MyMath.round(totalevent));
				if (invoiceCrud.getTax_edit() != 0) {
					invoiceCrud.setThue(invoiceCrud.getTax_edit());
				} else {
					invoiceCrud.setThue(MyMath.round(invoiceCrud.getTongtien() * invoiceCrud.getTax_value()));
				}
	...
	public void deleteInvoiceDetail() {
		...
		try {
			...
			if (wrapInvoiceDetailSelect != null) {
				// Kiem tra co so hoa don khong cap nhat duoc phieu
				...
				if (detail.getInvoice() != null
						&& allowDelete(detail.getInvoice().getInvoice_date(), detail.getInvoice().getCreated_by())) {
					// Phiếu đã chuyển hóa đơn điện tử
					if (detail.getInvoice().getStatus() == 1) {
						Invoice inv = invoiceService.findById(detail.getInvoice().getId());
						Customer customer = inv.getCustomer();
						if (!"CH103".equals(customer.getCustomer_code())
								&& !"CH034".equals(customer.getCustomer_code()))
							if (!allowUserAdmin()) {
								status = true;
							}
					}
				...
				if (status) {
					warning("Phiếu đã có số hóa đơn không xóa được.");
				} else {
	...
	public void acceptDelInvoiceDetail() {
		...
		Notify notify = new Notify(FacesContext.getCurrentInstance());
		try {
			if (allowDelete(wrapInvoiceDetailSelect.getInvoice_detail().getInvoice().getInvoice_date(),
	...
	public void deleteInvoice() {
		...
		try {
			...
			if (invoiceSelect != null) {
				Invoice iv = invoiceService.findById(invoiceSelect.getId());
				if (iv != null) {
					if (allowDelete(iv.getInvoice_date(), iv.getCreated_by())) {
						if (iv.getStatus() == 1) {
	...
	public void deleteInvoiceFromBean(long invoice_id) {
		try {
			...
			if (allowDelete(invoice.getInvoice_date(), invoice.getCreated_by())) {

				WrapInvoiceDelInfo t1 = new WrapInvoiceDelInfo(invoice_id, account.getMember().getId(),
						account.getMember().getName());
				// chuẩn bị dữ liệu để delete
				List<String> list = new ArrayList<>();
				invoiceService.prepareDataDelInvoice(invoice_id, list);
				Message message = new Message();
				int code = processLogicInvoiceService.deleteInvoiceMaster(t1, message, getAccount().getUserName());
			} else {
				executeScript(
						"swaldesigntimer('Cảnh báo!', 'Tài khoản này không có quyền thực hiện hoặc tháng đã khoá!','warning',2000);");
			}
	...
	public void exportIEInvoiceListExcel() {
		try {
			...
			for (int i = 0; i < size; i++) {
				idInvoices.add(listInvoice.get(i).getId());
			}
			List<InvoiceDetail> listInvoiceDetail = new ArrayList<>();
			invoiceDetailService.selectByInvoices(idInvoices, listInvoiceDetail);

			// xuat file excel.
			if (listInvoiceDetail.size() > 0) {
				List<Object[]> results = new ArrayList<Object[]>();
				Object[] title = { "Số CT", "Ngày", "Mã KH", "Mã kho", "Mã XN", "Số HĐ", "Mã SP", "Tên SP", "Số lượng",
						"ID", "Số xe", "Đơn giá", "Thành tiền", "Đơn giá NT", "Hệ số thuế", "Số serie", "Số ĐH",
						"Mã SPĐH", "Ghi chú" };
				results.add(title);
				for (InvoiceDetail it : listInvoiceDetail) {
					String cus = it.getInvoice().getCustomer() != null
							...
							: "";
					String wah = it.getInvoice().getWarehouse() != null ? it.getInvoice().getWarehouse().getCode() : "";
					String ie = it.getInvoice().getIe_categories() != null
							? it.getInvoice().getIe_categories().getCode()
							: "";
					String ct = it.getInvoice().getContract() != null ? it.getInvoice().getContract().getContract_code()
							: "";
					String car = it.getInvoice().getCar() != null ? it.getInvoice().getCar().getLicense_plate() : "";
					Object[] row = { Objects.toString(it.getInvoice().getVoucher_code(), ""),
							it.getInvoice().getInvoice_date(), cus, wah, ie, ct, it.getProduct().getProduct_code(),
							it.getProduct().getProduct_name(), it.getQuantity(), it.getInvoice().getId(), car,
							it.getUnit_price(), it.getTotal(), it.getForeign_unit_price(),
							it.getInvoice().getTax_value(), it.getInvoice().getInvoice_serie(),
							it.getInvoice().getOrder_voucher(), it.getProductdh_code(), it.getInvoice().getNote() };
					results.add(row);
				}
		...
		try {
			...
			if (datas.size() > 0) {
				...
				Object[] title = { "Mã KH xuất", "Số CT", "Ngày", "Mã KH", "Mã kho", "Mã XN", "Số HĐ", "Mã SP",
						...
						"Quy đổi SP chính", "Mã SP KM", "Quy đổi SP KM", "Thủ Kho", "Ghi chú CTKM", "Số lượng pallet",
						"Số HĐ thương mại", "Nhãn hàng", "Ngày HĐ gốc", "Mã khu vực", "Tên khu vực" };
				results.add(title);
				String chinhanh = getDatabase();
				String nameCN = "";
				String makhachhang = "";
				...
				for (Object[] it : datas) {
					...
					StringBuffer hesoquydoispchinh = new StringBuffer();
					StringBuffer maspkm = new StringBuffer();
					StringBuffer hesoquydoispkm = new StringBuffer();
					List<ProductKM> productKMs = productKMService.findByCodeProductMain((String) it[7]);
					for (int i = 0; i < productKMs.size(); i++) {
						if (productKMs.get(i).isSpchinh()) {
							if (!"".equals(maspchinh.toString())) {
								maspchinh.append(";");
								hesoquydoispchinh.append(";");
							}
							maspchinh.append(productKMs.get(i).getPromotion_product().getProduct_code());
							hesoquydoispchinh.append(productKMs.get(i).getPromotion_product().getFactor());
						} else {
							if (!"".equals(maspkm.toString())) {
								maspkm.append(";");
								hesoquydoispkm.append(";");
							}
							maspkm.append(productKMs.get(i).getPromotion_product().getProduct_code());
							hesoquydoispkm.append(productKMs.get(i).getPromotion_product().getFactor());
						}
					}
					it[51] = maspchinh;
					it[52] = hesoquydoispchinh;
					it[53] = maspkm;
					it[54] = hesoquydoispkm;
					...
					// }
					try {
						long idhdgoc = Long.parseLong(it[60].toString());
						if (idhdgoc != 0) {
							Invoice iv = invoiceService.findByIdSafe(idhdgoc);
							Date ngayhdgoc = iv.getInvoice_date();
							it[60] = ngayhdgoc;
						}
					} catch (Exception e) {
					}
		...
		try {
			...
			if (datas != null && datas.size() > 0) {
				...
				String[] title = { "id", "Mã KH xuất", "Số CT", "Ngày", "Mã KH", "Mã XN", "Số HĐ", "Mã SP", "Số lượng",
						...
						"Mã lô hàng", "Mã tra cứu", "Mã sản phẩm ĐH", "refID" };
				results.add(title);
				...
				for (int i = 0; i < datas.size(); i++) {
					try {
						...
						dataitem[19] = datas.get(i)[19] == null ? "" : datas.get(i)[19].toString();
	...
	public void acceptDelInvoice() {
		...
		try {
			...
			if (iv != null) {
				if (allowDelete(iv.getInvoice_date(), iv.getCreated_by())) {
					WrapInvoiceDelInfo t1 = new WrapInvoiceDelInfo(iv.getId(), account.getMember().getId(),
							account.getMember().getName());
					// chuẩn bị dữ liệu để delete
					Message message = new Message();
					int code = processLogicInvoiceService.deleteInvoiceMaster(t1, message, getAccount().getUserName());
						...
						if ("CKDH".equals(product.getProduct_code())) {
							...
							if (pricingProgram != null) {
								...
								long idPriceSub = customerPricingProgramService.selectForCustomerSub(
										pricingProgram.getId(), invoiceCrud.getInvoice_date(), product.getId());
								if (idPriceSub != 0) {
							...
							if (caidgkm) {
								...
								json.addProperty("date", ToolTimeCustomer
										.convertDateToString(invoiceCrud.getInvoice_date(), "dd/MM/yyyy"));
		...
		try {
			if (invoiceCrud != null && invoiceCrud.getId() != 0) {
				if (wrapInvoiceDetailCrud != null) {
					...
					if (invoiceOld != null && allowUpdate(invoiceOld.getInvoice_date(), invoiceOld.getCreated_by())
							&& allowUpdate(invoiceCrud.getInvoice_date(), invoiceCrud.getCreated_by())) {
						// Phiếu đã chuyển hóa đơn điện tử
					...
					if (status) {
						warning("Phiếu đã có số hóa đơn không cập nhật được.");
					} else {
						InvoiceDetail invoiceDetail = wrapInvoiceDetailCrud.getInvoice_detail();
						...
						if (product != null) {
							...
							if (invoiceDetail.getId() == 0) {
								if (allowSave(invoiceCrud.getInvoice_date(), invoiceCrud.getCreated_by())) {
				...
				if (ieCategories != null && customer != null && invoiceDate != null && warehouse != null
					...
					if (statusNVC == false) {
						...
						if (listWrapInvoiceDetail.size() != 0)
							sumInvoice();
						if (invoiceCrud.getId() == 0) {
							if (allowSave(invoiceCrud.getInvoice_date())) {
								if (invoiceService.insert(clone) == 0) {
									...
									if (invoiceTemp != null) {
										if (listInvoice == null)
											search();
										listInvoice.add(0, new InvoiceDTO3(invoiceTemp));
							...
							Invoice invoiceOld = invoiceService.findByIdSafe(invoiceCrud.getId());
							if (invoiceOld != null
									&& allowUpdate(invoiceOld.getInvoice_date(), invoiceOld.getCreated_by())
									&& allowUpdate(invoiceCrud.getInvoice_date(), invoiceCrud.getCreated_by())) {
								// Phiếu đã chuyển hóa đơn điện tử
								if (invoiceOld.getStatus() == 1) {
									if (!allowUserAdmin()) {
										...
										invoiceCrud.setInvoice_date(invoiceOld.getInvoice_date());
									...
									boolean cothaydoi = invoiceCrud.equals2(invoiceOld);
									// Kiểm tra thay đổi thông tin khi đã xuất
									// hóa đơn
									System.out.println("Có thay đổi: " + cothaydoi);
								}
								...
								if (invoiceService.updateSelect(invoiceCrud.clone(), getAccount().getUserName()) > 0) {
									...
									if (invoiceTemp != null) {
										...
										if (index != -1) {
											listInvoice.set(index, new InvoiceDTO3(invoiceTemp));
										}
										invoiceCrud = invoiceTemp.clone();
									}
								} else {
	...
	public void capNhatMaTraCuuHDDTDS() {
		PrimeFaces current = PrimeFaces.current();
		List<InvoiceDTO3> invoiceChoose = new ArrayList<InvoiceDTO3>();
		for (int i = 0; i < listInvoice.size(); i++) {
			if (listInvoice.get(i).isChonphieu()) {
				invoiceChoose.add(listInvoice.get(i));
			}
		}
		if (invoiceChoose.size() != 0) {
			for (int i = 0; i < invoiceChoose.size(); i++) {
				this.invoiceCrud = invoiceService.findById(invoiceChoose.get(i).getId());
				capNhatMaTraCuuHDDT();
			}
			createdNew();
			search();
		...
		if (listInvoice != null)
			for (int i = 0; i < listInvoice.size(); i++) {
				if (listInvoice.get(i).isChonphieu()) {
					...
					invoiceChoose.add(iv);
					if (hoadonTMDT == false)
						if ("$".equals(iv.getIe_categories().getCode())) {
							hoadonTMDT = true;
						}
				}

			}
		if (invoiceChoose.size() > 20 && (hoadonTMDT == false || invoiceChoose.size() > 300)) {
			current.executeScript(
					"swaldesigntimer2('Cảnh báo','Không chuyển được quá 20 hóa đơn thường hoặc 300 hóa đơn TMĐT.','warning',4000)");
		} else {
			if (invoiceChoose.size() != 0) {
				for (int i = 0; i < invoiceChoose.size(); i++) {
					this.invoiceCrud = invoiceChoose.get(i);
					// System.out.println("Chuyen hoa don: "+this.invoiceCrud.getVoucher_code());
		...
		try {
			if ("LIXCO@2023HD".equals(passhoadonjson)) {
				...
				if (capnhatphieu) {
					...
					if (thongbao.isLoi() == false) {
						System.out.println("Chuyen hoa don tu json------- refID: " + datahoadonjsonRefId);
						String result = MisaInvoice.insertInvoice(thongbao.getDulieu(), eInvoiceService);
						try {
							Gson gson = new Gson();
							KetQua kqinsert = gson.fromJson(result, KetQua.class);
							if (kqinsert != null) {
								...
								noticeError(
										"Đã chuyển nhưng không định dạng được kết quả trả về (xem console): " + result);
								current.executeScript("console.log('" + result + "')");
							}
						} catch (Exception e) {
							noticeError("Đã chuyển nhưng không định dạng được kết quả trả về (xem console): " + result
									+ "-ERROR-" + e.getMessage());
							current.executeScript("console.log('" + result + "')");
						}
					} else {
	    ...
	    try {
	        if (invoiceCrud != null && invoiceCrud.getId() != 0) {
	            ...
	            if (invoiceLoad.getCarrier() == null) {
	                for (String code : MAKT_NVC) {
	                    if (invoiceLoad.getIe_categories().getCode().equals(code)) {
	                        statusNVC = false;
	                        break;
	                    }
	                }
	            ...
	            if (!statusNVC) {
	                error("Không chuyển được hóa đơn (ID: " + invoiceLoad.getId() + ") do không có người vận chuyển.");
	                return;
	            }

	            boolean capnhatphieu = true;
	            if (invoiceLoad.getRefId() != null && !invoiceLoad.getRefId().isEmpty()) {
	            ...
	            boolean isOnlineInvoice = false;
	            if (!phieuxuatkho) {
	                for (String code : MANX_ONLINE) {
	                    if (invoiceLoad.getIe_categories().getCode().equals(code)) {
	                        thongbao = InvoiceToJson.toJsonHoaDon2(invoiceLoad, eInvoiceService, invoiceService,
	                            invoiceDetailService, ieInvoiceService, lamtron, getDatabase(),
	                            getAccount().getUserName(), productService);
	                        isOnlineInvoice = true;
	                        break;
	                    }
	                }
	                if (!isOnlineInvoice) {
	                    thongbao = InvoiceToJson.toJsonHoaDon(invoiceLoad, eInvoiceService, invoiceService,
	                        invoiceDetailService, ieInvoiceService, lamtron, getDatabase(),
	                        getAccount().getUserName(), productService);
	                }
	            } else {
	                thongbao = InvoiceToJson.toJsonXuatKho(invoiceLoad, eInvoiceService, invoiceService,
	                    invoiceDetailService, ieInvoiceService, lamtron, getAccount().getUserName());
	            }

	            if (thongbao == null) {
	                noticeError("Không thể tạo JSON hóa đơn cho hóa đơn ID: " + invoiceLoad.getId());
	                current.executeScript(
	                    "swaldesigntimer2('Lỗi','Không thể tạo JSON hóa đơn, nhấn F12 để xem chi tiết','error',2000)");
	                return;
	            }

	            if (!thongbao.isLoi()) {
	                try {
	                    System.out.println("Chuyển hóa đơn-------" + invoiceLoad.getId() + "\t refID: "
	                    ...
	                    System.out.println("\tDone" + invoiceLoad.getId() + "\t refID: " + invoiceLoad.getRefId());
	                } catch (Exception e) {
	                    logger.error("Lỗi khi lưu JSON hóa đơn: " + e.getMessage(), e);
	                }

	                String result = MisaInvoice.insertInvoice(thongbao.getDulieu(), eInvoiceService);
	                try {
	                    current.executeScript("console.log('KẾT QUẢ CHUYỂN: " + result + "')");
	                    Gson gson = new Gson();
	                    KetQua kqinsert = gson.fromJson(result, KetQua.class);
	                    if (kqinsert != null) {
	                        if (kqinsert.getCode() == 200) {
	                            if ("true".equals(kqinsert.getThamso().getSuccess())) {
	                                ...
	                                String jsonreturn = kqinsert.getThamso().getData();
	                                try {
	                                    ObjectMapper mapper = new ObjectMapper();
	                                    JsonNode root = mapper.readTree(jsonreturn);
	                                    JsonNode firstNode = root.elements().next();
	                                    String errorMessage = firstNode.get("ErrorMessage").asText();
	                                    if (errorMessage == null || errorMessage.isEmpty() || "null".equals(errorMessage)) {
	                                        success("Đã nạp hóa đơn thành công");
	                                    } else {
	                                        current.executeScript("swaldesigntimer2('Không thành công','"
	                                            + errorMessage + "','error',2000)");
	                                        current.executeScript("console.log('Thông báo lỗi errorMessage:"
	                                            + errorMessage + "; JSON kết quả:" + result + "')");
	                                    }
	                                } catch (Exception e) {
	                                    logger.error("Lỗi khi phân tích JSON trả về: " + e.getMessage(), e);
	                                    current.executeScript(
	                                        "swaldesigntimer2('Không thành công','Xảy ra lỗi, nhấn F12 để xem chi tiết','error',2000)");
	                                    current.executeScript("console.log('" + result + e.getMessage() + "')");
	                                }
	                            } else {
	                                current.executeScript(
	                                    "swaldesigntimer2('Không thành công','Xảy ra lỗi, nhấn F12 để xem chi tiết','error',2000)");
			...
			if (part != null) {
				...
				lv1: while (rows.hasNext()) {
					...
					if (customer != null && ngay != null && sochungtu != null) {
						...
						if (invoice != null) {
							List<InvoiceDetail> listInvoiceDetail = new ArrayList<>();
							invoiceDetailService.selectByInvoice(invoice.getId(), listInvoiceDetail);
							double total = listInvoiceDetail.stream().mapToDouble(x -> x.getTotal()).sum();
	...
	// cap nhat thanh toan
	public void xacNhanThanhToan(InvoiceDTO3 invoiceDTO3) {
		try {
			int result = invoiceService.updatePayment(invoiceDTO3.getId(), getAccount().getUserName(),
					invoiceDTO3.isPayment());
			if (result == 1) {
				InvoiceReqInfo invoiceReqInfo = new InvoiceReqInfo();
				invoiceService.selectById(invoiceDTO3.getId(), invoiceReqInfo);
				Invoice invoiceTemp = invoiceReqInfo.getInvoice();
				if (invoiceTemp != null) {
					int index = -1;
					for (int i = 0; i < listInvoice.size(); i++) {
	...
	public void loadExcelNapHoaDon(FileUploadEvent event) {
		...
		try {
			...
			if (part != null) {
				...
				List<DuLieuHoaDonOnline> hoaDonOnlines = new ArrayList<DuLieuHoaDonOnline>();
				Date now = new Date();
				while (rows.hasNext()) {
					...
					while (cells.hasNext()) {
						...
						case 3:// masanpham
							try {
								masp = MyUtilExcel.getCellValueMaSP(cell);
							} catch (Exception e) {
							}
							break;
						case 5:// dongia
							try {
								dongia = Double.parseDouble(Objects.toString(MyUtilExcel.getCellValue(cell), "0"));
							} catch (Exception e) {
							}
							break;
						...
						case 7:// thanhtien
							try {
								thanhtien = Double.parseDouble(Objects.toString(MyUtilExcel.getCellValue(cell), "0"));
							} catch (Exception e) {
							}
							break;
						...
						case 8:// Chietkhau
							try {
								chietkhau = Double.parseDouble(Objects.toString(MyUtilExcel.getCellValue(cell), "0"));
							} catch (Exception e) {
							}
							break;
						...
						case 10:// ma kh
							...
							break;
						case 11:// loai nhap xuat
							try {
								loainhapxuat = Objects.toString(MyUtilExcel.getCellValue(cell), "");
							} catch (Exception e) {
							}
							break;
						case 12:// loai nhap xuat
							try {
								maspchinh = Objects.toString(MyUtilExcel.getCellValue(cell), "");
							} catch (Exception e) {
							}
							break;
						}
					...
					if (ngay != null && madonhang != null && masp != null) {
						DuLieuHoaDonOnline hd = new DuLieuHoaDonOnline(ngay, madonhang, masp, dongia, soluong,
								thanhtien, chietkhau, thanhtientong, makh, loainhapxuat, maspchinh);
						hoaDonOnlines.add(hd);
					}
				}
				List<Invoice> invoiceLuus = new ArrayList<Invoice>();
				if (hoaDonOnlines.size() != 0) {
					Map<String, Map<String, Map<String, Map<String, List<DuLieuHoaDonOnline>>>>> hds = hoaDonOnlines
							.stream()
							.collect(Collectors.groupingBy(DuLieuHoaDonOnline::getMadonhang,
									Collectors.groupingBy(DuLieuHoaDonOnline::getMakh,
											Collectors.groupingBy(DuLieuHoaDonOnline::getMasp,
													Collectors.groupingBy(DuLieuHoaDonOnline::getDonGia_Maspchinh)))));

					// CK/TM (id 4)
					PaymentMethod pm = paymentMethodService.findById(4);
					// $: ban hang online
					IECategories iecater = ieCategoriesService.selectByCode("$");
					// p: gioi thieu sản phẩm
					IECategories iecaterKMp = ieCategoriesService.selectByCode("P");
					// &: xuatKM tai kho online
					IECategories iecaterKM = ieCategoriesService.selectByCode("&");
					// F: Kho san pham
					Warehouse wh = warehouseService.selectByCode("F");
					Carrier carrier = carrierService.selectByCode("X");
					// HÀNG
					// ONLINE
					double hesothue = GiaTriMacDinh.HESOTHUE;

					for (String madh : hds.keySet()) {
						Map<String, Map<String, Map<String, List<DuLieuHoaDonOnline>>>> hds1 = hds.get(madh);
						for (String makh : hds1.keySet()) {
							Customer customer = customerService.selectByCodeNotDisable(makh);
							if (customer == null) {
								dulieukhongnapduoc.add("Không có mã khách hàng hoặc KH không còn hoạt động: " + makh);
							} else {
								// Lay chuong trinh đơn giá
								JsonObject json = new JsonObject();
								json.addProperty("date",
										ToolTimeCustomer.convertDateToString(new Date(), "dd/MM/yyyy"));
								json.addProperty("customer_id", customer.getId());
								CustomerPricingProgramReqInfo t = new CustomerPricingProgramReqInfo();
								customerPricingProgramService.selectForCustomer(JsonParserUtil.getGson().toJson(json),
										t);
								PricingProgram dongiamoi = t.getCustomer_pricing_program() == null ? null
										: t.getCustomer_pricing_program().getPricing_program();
								...
								double chietkhau = 0;
								List<InvoiceDetail> invoiceDetails = new ArrayList<InvoiceDetail>();
								List<InvoiceDetail> invoiceDetailKMs = new ArrayList<InvoiceDetail>();
								List<InvoiceDetail> invoiceDetailDonGia0MaPs = new ArrayList<InvoiceDetail>();
								Map<String, Map<String, List<DuLieuHoaDonOnline>>> hds2 = hds1.get(makh);
								for (String masp : hds2.keySet()) {
									Map<String, List<DuLieuHoaDonOnline>> hds3 = hds2.get(masp);
									for (String dongia_SPChinh : hds3.keySet()) {

										List<DuLieuHoaDonOnline> hds4 = hds3.get(dongia_SPChinh);
										double dongia = hds4.get(0).getDongia();
										String loainx = hds4.get(0).getLoainhapxuat();
										// Kiểm tra nếu đơn giá bằng 0 và mã P->
										// xuat hoa
										// don gioi thieu san pham
										if (0.0 == dongia) {
											double soluong = 0;
											Map<String, List<DuLieuHoaDonOnline>> hdPhanLoais = hds4.stream().collect(
													Collectors.groupingBy(DuLieuHoaDonOnline::getLoainhapxuat));
											for (String loainhapxuat : hdPhanLoais.keySet()) {

												List<DuLieuHoaDonOnline> hdbang0s = hdPhanLoais.get(loainhapxuat);
												for (int i = 0; i < hdbang0s.size(); i++) {
													soluong += hdbang0s.get(i).getSoluong();
													chietkhau += hdbang0s.get(i).getChietkhau();
												}
												...
												if (product != null) {
													...
													detail.setProductdh_code(hdbang0s.get(0).getMaspchinh());
													if ("P".equalsIgnoreCase(loainhapxuat)) {
														// cài don giá
														if (dongiamoi != null) {
															boolean statusPrice = true;
															PricingProgramDetailReqInfo t1 = new PricingProgramDetailReqInfo();
															long idPriceSub = customerPricingProgramService
																	.selectForCustomerSub(dongiamoi.getId(),
																			invoiceCrud.getInvoice_date(),
																			product.getId());
															if (idPriceSub != 0) {

																pricingProgramDetailService.findSettingPricing(
																		idPriceSub, product.getId(), t1);

																if (t1.getPricing_program_detail() != null) {
																	if (t1.getPricing_program_detail()
																			.getUnit_price() != 0) {
																		detail.setUnit_price(
																				t1.getPricing_program_detail()
																						.getUnit_price());
																		statusPrice = false;
																	}
																}
															...
															if (statusPrice) {
																pricingProgramDetailService.findSettingPricing(
																		dongiamoi.getId(), product.getId(), t1);
																if (t1.getPricing_program_detail() != null) {
																	if (t1.getPricing_program_detail()
																			.getUnit_price() != 0) {
																		detail.setUnit_price(
															...
															dulieukhongnapduoc.add(
																	"Sản phẩm: " + detail.getProduct().getProduct_code()
																			+ " xuất mã P (mã khách hàng: "
																			+ customer.getCustomer_code()
																			+ ") không tìm thấy chương trình đơn giá");
														}
													}
													detail.setTotal(MyMath
															.round(detail.getQuantity() * detail.getUnit_price()));
													detail.setNapdulieu("NapTMDT");
													Object[] dutk = dutonkho(detail);
													if ((boolean) dutk[0]) {
														if ("P".equalsIgnoreCase(loainhapxuat)) {
															invoiceDetailDonGia0MaPs.add(detail);
														}
													} else {
														dulieukhongnapduoc.add(
																"Sản phẩm: " + detail.getProduct().getProduct_code()
																		+ " không đủ tồn kho (xuất: "
																		+ MyUtilEJB.dinhdangso(detail.getQuantity())
																		+ " -> tồn kho: "
																		+ MyUtilEJB.dinhdangso((double) dutk[1]) + ")");
													}
												} else {
											...
											if (product != null) {
												...
												detail.setTotal(
														MyMath.round(detail.getQuantity() * detail.getUnit_price()));
												detail.setNapdulieu("NapTMDT");
												detail.setProductdh_code(hds4.get(0).getMaspchinh());
												Object[] dutk = dutonkho(detail);
												if ((boolean) dutk[0]) {
													if ("$".equals(loainx)) {
														invoiceDetails.add(detail);

													} else if ("&".equals(loainx)) {
														invoiceDetailKMs.add(detail);
													}

												} else {
													dulieukhongnapduoc
															.add("Sản phẩm: " + detail.getProduct().getProduct_code()
																	+ " không đủ tồn kho (xuất: "
																	+ MyUtilEJB.dinhdangso(detail.getQuantity())
																	+ " -> tồn kho: "
																	+ MyUtilEJB.dinhdangso((double) dutk[1]) + ")");
								...
								if (chietkhau != 0) {
									// CKDH:chiet khau don hang
									Product product = productService.selectByCode("CKDH");
									if (product != null) {
										InvoiceDetail detail = new InvoiceDetail();
										detail.setCreated_by_id(getAccount().getMember().getId());
										detail.setCreated_by(getAccount().getMember().getName());
								...
								// Don gia = 0, mã xuat nhap & xuat tại kho
								// online
								if (invoiceDetailKMs.size() != 0) {
									Invoice invoiceKM = new Invoice();
									invoiceKM.setNaponline(true);
									invoiceKM.setCreated_by_id(getAccount().getMember().getId());
									invoiceKM.setCreated_by(getAccount().getMember().getName());
									invoiceKM.setCreated_date(new Date());
									invoiceKM.setCustomer(customer);
									invoiceKM.setInvoice_date(now);
									invoiceKM.setDelivery_date(now);
									invoiceKM.setPayment_method(pm);
									// &: xuat Km tại kho online
									invoiceKM.setIe_categories(iecaterKM);
									invoiceKM.setWarehouse(wh);
									invoiceKM.setTax_value(hesothue);// he
																		// so
																		// thue
																		// =0.1
									invoiceKM.setVoucher_code(madh);
									invoiceKM.setPo_no(madh);
									invoiceKM.setExported(true);
									invoiceKM.setCarrier(carrier);
									double totalevent = invoiceDetailKMs.stream().mapToDouble(f -> f.getTotal()).sum();
									invoiceKM.setTongtien(MyMath.round(totalevent));
									invoiceKM.setThue(MyMath.round(totalevent * invoiceKM.getTax_value()));
									invoiceKM.setNote("Hàng khuyến mãi không thu tiền");

									if (allowSave(invoiceKM.getInvoice_date())) {
										invoiceKM.setList_invoice_detail(invoiceDetailKMs);
										invoiceLuus.add(invoiceKM);
									} else {
										dulieukhongnapduoc.add(
												"Tài khoản này không có quyền thực hiện hoặc tháng đã khoá: " + madh);
									}
								}
								// Don gia = 0, mã xuat nhap P xuat gioi
								// thieu
								// sản phẩm
								if (invoiceDetailDonGia0MaPs.size() != 0) {
									Invoice invoiceKM = new Invoice();
									invoiceKM.setNaponline(true);
									invoiceKM.setCreated_by_id(getAccount().getMember().getId());
									invoiceKM.setCreated_by(getAccount().getMember().getName());
									...
									invoiceKM.setDelivery_date(now);
									invoiceKM.setPayment_method(pm);
									// P: xuat giới thiệu sản phẩm
									invoiceKM.setIe_categories(iecaterKMp);
									invoiceKM.setWarehouse(wh);
									invoiceKM.setTax_value(hesothue);
									invoiceKM.setPricing_program(dongiamoi);
									invoiceKM.setVoucher_code(madh);
									invoiceKM.setPo_no(madh);
									invoiceKM.setExported(true);
									invoiceKM.setCarrier(carrier);
									double totalevent = invoiceDetailDonGia0MaPs.stream().mapToDouble(f -> f.getTotal())
											.sum();
									invoiceKM.setTongtien(MyMath.round(totalevent));
									invoiceKM.setThue(MyMath.round(totalevent * invoiceKM.getTax_value()));
									invoiceKM.setNote("Hàng giới thiệu sản phẩm không thu tiền");

									if (allowSave(invoiceKM.getInvoice_date())) {
										invoiceKM.setList_invoice_detail(invoiceDetailDonGia0MaPs);
										invoiceLuus.add(invoiceKM);
									} else {
										dulieukhongnapduoc.add(
												"Tài khoản này không có quyền thực hiện hoặc tháng đã khoá: " + madh);
								...
								if (invoiceDetails.size() != 0) {
									...
									if (allowSave(invoice.getInvoice_date())) {
										invoice.setList_invoice_detail(invoiceDetails);
										invoiceLuus.add(invoice);
										// khóa lại nếu phát sinh lỗi không
										// cho
										// lưu cả danh sách
				...
				if (dulieukhongnapduoc.size() != 0) {
					...
					if (invoiceLuus.size() == 0) {
						noticeError("Không đọc được dữ liệu trong danh sách.");
					} else {
						for (int i = 0; i < invoiceLuus.size(); i++) {
							List<InvoiceDetail> invoiceDetails = invoiceLuus.get(i).getList_invoice_detail();
							invoiceLuus.get(i).setList_invoice_detail(null);
							processLogicInvoiceService.createInvoiceByExcelOnline(invoiceLuus.get(i), invoiceDetails);
						}
						notify.success();
					}
				}
			...
			logger.error("invoiceBean.loadExcelThanhToan:" + e.getMessage(), e);
			noticeError("Lỗi đọc dữ liệu: " + e.getMessage());
		}
	}
			...
			if (part != null) {
				...
				while (rows.hasNext()) {
					...
					while (cells.hasNext()) {
						...
						case 0:
							try {
								if (MyUtilExcel.getCellValue(workBook, cell).getClass().equals(Double.class)) {
									ngay = DateUtil.getJavaDate((double) MyUtilExcel.getCellValue(workBook, cell));
								} else {
									ngay = (Date) MyUtilExcel.getCellValue(workBook, cell);
								}
								break;
							} catch (Exception e) {
								e.printStackTrace();
								String ngaydathang = Objects.toString(MyUtilExcel.getCellValue(workBook, cell), null);
								if (ngaydathang != null && !"".equals(ngaydathang)) {
									ngay = ToolTimeCustomer.convertStringToDate(ngaydathang, "dd/MM/yyyy");
								}
								break;
							}
						case 1:
						...
						case 9:// slpallet
							try {
								slpallet = Double.parseDouble(Objects.toString(MyUtilExcel.getCellValue(cell), "0"));
							} catch (Exception e) {
							}
							break;
						case 10:// PO
							try {
								PO = Objects.toString(MyUtilExcel.getCellValue(cell));
							} catch (Exception e) {
							}
							break;
						}
					...
					if (masp != null && makh != null && !"".equals(makh)) {
						DuLieuHoaDonVSIP hd = new DuLieuHoaDonVSIP(ngay, sochungtu, makh, masp, soluong, malohang,
								ghichuxuat, nhomtheohd, slpallet);
						hd.setPO(PO);
						hoaDonVSIPs.add(hd);
					}
				}
				if (hoaDonVSIPs.size() != 0) {
					Map<String, Map<Integer, List<DuLieuHoaDonVSIP>>> hds = hoaDonVSIPs.stream()
						...
						for (Integer hoadon : hds1.keySet()) {
							...
							for (int i = 0; i < hds4.size(); i++) {
								...
								if (product != null) {
									...
									detail.setSlpallet(hds4.get(i).getSlpallet());
									...
									dulieukhongnapduoc.add("Không tìm thấy mã sản phẩm: " + hds4.get(i).getMasp());
								}
							}
							...
							if (customer != null) {
								if (invoiceDetails.size() != 0) {
									Invoice invoice = new Invoice();
									if (hds4.get(0).getNgay() != null) {
										invoice.setInvoice_date(hds4.get(0).getNgay());
									} else {
										invoice.setInvoice_date(now);
									}
									invoice.setVoucher_code(hds4.get(0).getSochungtu());
									invoice.setNaponline(true);
									invoice.setCreated_by_id(getAccount().getMember().getId());
									invoice.setCreated_by(getAccount().getMember().getName());
									invoice.setCreated_date(new Date());
									...
									invoice.setPo_no(hds4.get(0).getPO());
									if (allowSave(invoice.getInvoice_date())) {
										processLogicInvoiceService.createInvoiceByExcelOnline(invoice, invoiceDetails);
									} else {
										dulieukhongnapduoc.add(
	...
	// Nap hoa dopn excel mau 2
	public void loadExcelNapHoaDonMau2(FileUploadEvent event) {
		dulieukhongnapduoc = new ArrayList<String>();
		Notify notify = new Notify(FacesContext.getCurrentInstance());
		try {
			UploadedFile part = event.getFile();
			if (part != null) {
				...
				List<DuLieuHoaDonOnline2> hoaDonOnlines = new ArrayList<DuLieuHoaDonOnline2>();
				Date now = new Date();
				while (rows.hasNext()) {
					...
					while (cells.hasNext()) {
						...
						case 8:
							try {
								manvvc = Objects.toString(MyUtilExcel.getCellValue(cell), "");
							} catch (Exception e) {
							}
							break;
						case 9:
							try {
								soxe = Objects.toString(MyUtilExcel.getCellValue(cell), "");
							} catch (Exception e) {
							}
							break;
						case 10:
						...
						case 11:
							try {
								sophieuvanchuyen = Objects.toString(MyUtilExcel.getCellValue(cell), "").replace(".0",
										"");
							} catch (Exception e) {
							}
							break;
						case 12:
							try {
								noiden = Objects.toString(MyUtilExcel.getCellValue(cell), "").replace(".0", "");
							} catch (Exception e) {
								e.printStackTrace();
							}
							break;
						case 13:
							try {
								malohang = Objects.toString(MyUtilExcel.getCellValue(cell), "").replace(".0", "");
							} catch (Exception e) {
								e.printStackTrace();
							}
							break;
					...
					if (ngay != null && madonhang != null && masp != null && makh != null) {
						DuLieuHoaDonOnline2 hd = new DuLieuHoaDonOnline2(ngay, madonhang, masp, soluong, makh,
								maxuatnhap, manvvc, soxe, htthanhtoan, sophieuvanchuyen, noiden, malohang);
						hoaDonOnlines.add(hd);
					}
				}
				List<Invoice> invoiceLuus = new ArrayList<Invoice>();
				if (hoaDonOnlines.size() != 0) {
					Map<String, Map<String, Map<String, List<DuLieuHoaDonOnline2>>>> hds = hoaDonOnlines.stream()
							.collect(Collectors.groupingBy(DuLieuHoaDonOnline2::getMadonhang,
									Collectors.groupingBy(DuLieuHoaDonOnline2::getMakh,
											Collectors.groupingBy(DuLieuHoaDonOnline2::getMaxuatnhap))));
					...
					for (String madh : hds.keySet()) {
						Map<String, Map<String, List<DuLieuHoaDonOnline2>>> hds1 = hds.get(madh);
						for (String makh : hds1.keySet()) {
							Customer customer = customerService.selectByCodeNotDisable(makh);
							if (customer == null) {
								dulieukhongnapduoc.add("Không có mã khách hàng hoặc KH không còn hoạt động: " + makh);
								...
								PricingProgram dongiamoi = t.getCustomer_pricing_program() == null ? null
										: t.getCustomer_pricing_program().getPricing_program();
								List<InvoiceDetail> invoiceDetails = new ArrayList<InvoiceDetail>();
								Map<String, List<DuLieuHoaDonOnline2>> hds2 = hds1.get(makh);
								for (String maxuatnhap : hds2.keySet()) {
									List<DuLieuHoaDonOnline2> hds4 = hds2.get(maxuatnhap);
									for (int i = 0; i < hds4.size(); i++) {
										Product product = productService.selectByCode(hds4.get(i).getMasp());
										if (product != null) {
											InvoiceDetail detail = new InvoiceDetail();
											...
											if (dongiamoi != null) {
												...
												PricingProgramDetailReqInfo t1 = new PricingProgramDetailReqInfo();
												long idPriceSub = customerPricingProgramService.selectForCustomerSub(
														dongiamoi.getId(), invoiceCrud.getInvoice_date(),
														product.getId());
												if (idPriceSub != 0) {
													...
													if (t1.getPricing_program_detail() != null) {
														if (t1.getPricing_program_detail().getUnit_price() != 0) {
															detail.setUnit_price(
																	t1.getPricing_program_detail().getUnit_price());
															statusPrice = false;
														}
													}
												}
												if (statusPrice) {
													pricingProgramDetailService.findSettingPricing(dongiamoi.getId(),
															product.getId(), t1);
													if (t1.getPricing_program_detail() != null) {
														if (t1.getPricing_program_detail().getUnit_price() != 0) {
															detail.setUnit_price(
												...
												dulieukhongnapduoc
														.add("Sản phẩm: " + detail.getProduct().getProduct_code()
																+ " (mã khách hàng: " + customer.getCustomer_code()
																+ ") -> không tìm thấy chương trình đơn giá");
											}

											detail.setTotal(
													MyMath.round(detail.getQuantity() * detail.getUnit_price()));
											detail.setNapdulieu("Napexcelmau2");
											Object[] dutk = dutonkho(detail);
											if ((boolean) dutk[0]) {
												invoiceDetails.add(detail);
											} else {
											...
											dulieukhongnapduoc
													.add("Không tìm thấy mã sản phẩm: " + hds4.get(i).getMasp());
										}
									}
									if (invoiceDetails.size() != 0) {
										Invoice invoice = new Invoice();
										...
										PaymentMethod pm = paymentMethodService
												.selectByCode(hds4.get(0).getHtthanhtoan());
										invoice.setPayment_method(pm);

										IECategories iecat = ieCategoriesService.selectByCode(maxuatnhap);
										if (iecat != null) {
											invoice.setIe_categories(iecat);
										}
										invoice.setPricing_program(dongiamoi);
										invoice.setWarehouse(wh);
										invoice.setTax_value(hesothue);
										invoice.setVoucher_code(madh);
										invoice.setPo_no(madh);
										invoice.setExported(true);
										Carrier carrier = carrierService.selectByCode(hds4.get(0).getManvvc());
										invoice.setCarrier(carrier);

										double totalevent = invoiceDetails.stream().mapToDouble(f -> f.getTotal())
												.sum();
										invoice.setTongtien(MyMath.round(totalevent));
										invoice.setThue(MyMath.round(totalevent * invoice.getTax_value()));

										invoice.setCar(carService.selectByCode(hds4.get(0).getSoxe()));
										invoice.setSophieuvc(hds4.get(0).getSophieuvanchuyen());
										invoice.setDelivery_pricing(
												deliveryPricingService.selectByPlaceCode(hds4.get(0).getNoiden()));
	...
	public void loadExcelNapHoaDonChietKhau(FileUploadEvent event) {
		...
		try {
			...
			if (part != null) {
				...
				List<DuLieuHoaDonChietKhau> hoaDonChietKhaus = new ArrayList<DuLieuHoaDonChietKhau>();
				Date now = new Date();
				while (rows.hasNext()) {
					...
					while (cells.hasNext()) {
						...
						case 7:
							try {
								manguoivanchuyen = Objects.toString(MyUtilExcel.getCellValue(cell), "");
							} catch (Exception e) {
							}
							break;
						}
					...
					if (masp != null && makh != null && !"".equals(makh) && maxuatnhap != null) {
						DuLieuHoaDonChietKhau hd = new DuLieuHoaDonChietKhau(masp, soluong, dongia, thanhtien, makh,
								maxuatnhap, htthanhtoan, manguoivanchuyen);
				...
				if (hoaDonChietKhaus.size() != 0) {
					...
					for (int i = 0; i < hoaDonChietKhaus.size(); i++) {
						PaymentMethod pm = paymentMethodService.selectByCode(hoaDonChietKhaus.get(i).getHtthanhtoan());
						IECategories iecater = ieCategoriesService
								.selectByCode(hoaDonChietKhaus.get(i).getMaxuatnhap());
						Product product = productService.selectByCode(hoaDonChietKhaus.get(i).getMasp());
						if (product != null) {
							InvoiceDetail detail = new InvoiceDetail();
							detail.setCreated_by_id(getAccount().getMember().getId());
							detail.setCreated_by(getAccount().getMember().getName());
							...
							Customer customer = customerService.selectByCode(hoaDonChietKhaus.get(i).getMakh());
							if (customer != null) {
								Invoice invoice = new Invoice();
								invoice.setNaponline(true);
								invoice.setCreated_by_id(getAccount().getMember().getId());
								...
								invoice.setTongtien(MyMath.round(totalevent));
								invoice.setThue(MyMath.round(totalevent * invoice.getTax_value()));
								if (hoaDonChietKhaus.get(i).getManguoivanchuyen() != null
										&& !hoaDonChietKhaus.get(i).getManguoivanchuyen().isEmpty()) {
									Carrier carrier = carrierService.selectByCode(hoaDonChietKhaus.get(i)
											.getManguoivanchuyen());
									invoice.setCarrier(carrier);
								}
								if (allowSave(invoice.getInvoice_date())) {
								...
								dulieukhongnapduoc.add("Không tìm thấy mã KH: " + hoaDonChietKhaus.get(i).getMakh());
							}
						} else {
							dulieukhongnapduoc.add("Không tìm thấy mã sản phẩm: " + hoaDonChietKhaus.get(i).getMasp());
						}
					}
					notify.success();
				} else {
	...
	public void loadExcelNapCTHoaDon(FileUploadEvent event) {
		try {
			...
			if (invoiceOld != null && allowUpdate(invoiceOld.getInvoice_date(), invoiceOld.getCreated_by())
				...
				if (invoiceOld.getStatus() == 1) {
					Customer customer = invoiceCrud.getCustomer();
					if (!"CH103".equals(customer.getCustomer_code()) && !"CH034".equals(customer.getCustomer_code()))
						if (!allowUserAdmin()) {
							status = true;
						}
				}
			...
			if (status) {
				...
				if (part != null) {
					...
					if (invoiceCrud != null) {
						...
						sumInvoice();// tinh lai tong cong
						executeScript("PF('dlnapchitiethoadon').hide();");
				...
				while (rows.hasNext()) {
					...
					if (i == 0) {
						...
						while (cells.hasNext()) {
							...
							case 12:
								try {
									thukho = Objects.toString(MyUtilExcel.getCellValue(workBook, cell), "");
								} catch (Exception e) {
								}
								break;
							}
						...
						if (status && soluongthung != 0) {
							DuLieuHoaDonDonHang hd = new DuLieuHoaDonDonHang(ngayhoadon, soluongthung, masp,
									iddonhangchitiet, iddonhang, soxe, hoadon, lohang, thukho);
					...
					for (String sohd : hds.keySet()) {
						...
						if (orderLixCrud == null) {
							...
							if (listWrapOrderDetailLixReqInfo.size() > 0) {
								...
								HeThong ht = heThongService.findById(1);
								String thukho = null;
								if (hdssub != null && hdssub.size() != 0) {
									thukho = hdssub.get(0).getThukho();
								}
								int code = processLogicInvoiceService.createInvoiceByExtensionOrder(data, listIdResult,
										messages, hdssub.get(0).getSoxe(), ht.isKiemton(), thukho);
								if (code == 0) {
									success();
								} else {
				...
				for (WrapOrderDetailLixReqInfo t : listWrapOrderDetailLixReqInfo) {
					...
					if (orderLixCrud.getPromotion_program() != null) {
						...
						lv1: for (PromotionProgramDetail ppd : list) {
							...
							// double quantity = (int) t.getExport_quantity() /
							// ppd.getBox_quatity()
							// * ppd.getSpecification();
							double quantity = (double) ((int) (t.getExport_quantity() / ppd.getBox_quatity()))
									* ppd.getSpecification();
	...
	@Getter
	@Setter
	private List<InvoiceDetail> invoiceDetailsDC;
	@Getter
	@Setter
	...
	public void openSplitDialog() {
		...
		Long invoiceId = listWrapInvoiceDetail.get(0).getInvoice_detail().getInvoice().getId();

		Invoice invoiceForCode = invoiceService.findById(invoiceId);
		this.voucherCode2Input = generateVoucherCode2(invoiceForCode);

		// Lấy các C2 đã có
		this.hoaDonC2List = hoaDonC2Service.findByInvoiceId(invoiceId);
		...
		for (HoaDonC2 c2 : hoaDonC2List) {
			...
			for (HoaDonChiTietC2 ct : c2.getChiTietC2List()) {
				Long key = ct.getInvoiceDetail().getId();
				splittedQtyMap.merge(key, ct.getQuantity(), Double::sum);
			}
		}

		// Lấy tất cả InvoiceDetail từ DB
		List<InvoiceDetail> invoiceDetails = hoaDonC2Service.findByInvoiceIdWithProduct(invoiceId);
		...
		hoaDonC2sDC = hoaDonC2Service.findByInvoiceId(invoiceId);
		invoiceDetailsDC = new ArrayList<InvoiceDetail>();
		invoiceDetailService.selectByInvoice(invoiceId, invoiceDetailsDC);
		hoaDonChiTietC2s = hoaDonC2Service.findChiTietByInvoiceId(invoiceId);
		mapSLTach = new HashMap<>();
		for (HoaDonChiTietC2 ct : hoaDonChiTietC2s) {
			String key = ct.getHoaDonC2().getVoucher_code2() + "|"
					+ ct.getInvoiceDetail().getProduct().getProduct_code();
	...
	public void saveSplit() {
		try {
			...
			Invoice invoice = listWrapInvoiceDetail.get(0).getInvoice_detail().getInvoice();
			...
			for (InvoiceDetailDTO3 dto : listDetailDTO3) {
				if (dto.getQuantityEdit() > 0) {
					...
					// lấy InvoiceDetail thực tế từ DB
					InvoiceDetail invDetail = invoiceDetailService.selectById(dto.getInvoiceDetailId());
					ct.setInvoiceDetail(invDetail);
					...
					c2.setVehicleNumber(dto.getVehicleNumber());
	...
	private String generateVoucherCode2(Invoice invoice) {
		String base = invoice.getVoucher_code();
		if (base == null || base.isEmpty())
			base = "CT";
		long seq = hoaDonC2Service.countByInvoiceId(invoice.getId()) + 1;
		return base + "-" + seq;
	}
	...
	public double getChenhLech(InvoiceDetail item) {
		double tongPhieu = getTongPhieuTao(item.getProduct().getProduct_code());
		return item.getQuantity() - tongPhieu;
	}

	public String getRowStyle(InvoiceDetail item) {
		double chenhlech = getChenhLech(item);
		if (chenhlech == 0) {
			return "row-green"; // Đủ
		} else if (chenhlech < 0) {
			return "row-red"; // Dư
		} else {
			return "row-yellow"; // Thiếu
		}
	}
...
//	@Getter
//	@Setter
//	private HoaDonChiTietC2 selectedChiTiet;   
//
//	@Getter
//	@Setter
//	private HoaDonChiTietC2 editChiTiet;       
//	
	
//	public void prepareEdit(HoaDonChiTietC2 ct) {
//	    if (ct != null) {
//	        try {
//	            this.selectedChiTiet = ct;
//	            this.editChiTiet = new HoaDonChiTietC2();
//	            this.editChiTiet.setId(ct.getId());
//	            this.editChiTiet.setProductdh_code(ct.getProductdh_code());
//	            this.editChiTiet.setProduct_name(ct.getProduct_name());
//	            this.editChiTiet.setQuantity(ct.getQuantity());
//	            this.editChiTiet.setVehicleNumber(ct.getVehicleNumber());
//	            this.editChiTiet.setBatchCode(ct.getBatchCode());
//	            this.editChiTiet.setCreatedDate(ct.getCreatedDate());
//	            this.editChiTiet.setCreatedUser(ct.getCreatedUser());
//
//	            PrimeFaces.current().executeScript("PF('dlgEditChiTiet').show();");
//	        } catch (Exception e) {
//	            FacesContext.getCurrentInstance().addMessage(null,
//	                new FacesMessage(FacesMessage.SEVERITY_ERROR, "Lỗi", "Không mở được dialog chỉnh sửa"));
//	        }
//	    }
//	}
//
...
//	    try {
//	    	hoaDonC2DetailService.update(editChiTiet);
//
//	        selectedChiTiet.setVehicleNumber(editChiTiet.getVehicleNumber());
//	        selectedChiTiet.setBatchCode(editChiTiet.getBatchCode());
//	        selectedChiTiet.setProduct_name(editChiTiet.getProduct_name());
//	        selectedChiTiet.setProductdh_code(editChiTiet.getProductdh_code());
//	        selectedChiTiet.setReal_quantity(editChiTiet.getReal_quantity());
//	        selectedChiTiet.setQuantity(editChiTiet.getQuantity());
//	        
//	        
//	        FacesContext.getCurrentInstance().addMessage(null,
//	            new FacesMessage(FacesMessage.SEVERITY_INFO, "Thành công", "Cập nhật chi tiết thành công"));
//	        PrimeFaces.current().executeScript("PF('dlgEditChiTiet').hide();");
//	    } catch (Exception e) {
//	        FacesContext.getCurrentInstance().addMessage(null,
//	            new FacesMessage(FacesMessage.SEVERITY_ERROR, "Lỗi", "Không thể lưu chi tiết"));
//	    }
//	}
	...
	public InvoiceDTO3 getInvoiceSelect() {
		return invoiceSelect;
	}

	public void setInvoiceSelect(InvoiceDTO3 invoiceSelect) {
		this.invoiceSelect = invoiceSelect;
	}

	public List<InvoiceDTO3> getListInvoice() {
		return listInvoice;
	}

	public void setListInvoice(List<InvoiceDTO3> listInvoice) {
	...
	public void chuyenDMSs() {
		...
		// if (invoiceChoose.size() != 0) {
		// for (int i = 0; i < invoiceChoose.size(); i++) {
		// Invoice iv = invoiceService.findById(invoiceChoose.get(i).getId());
		// boolean status = sendInvoiceToDMS(iv);
		// if (status)
		// countSuccess++;
		// }
		// notice("Đã chuyển " + countSuccess + "/" + invoiceChoose.size() +
		// " hóa đơn sang DMS");
		// } else {
		// current.executeScript("swaldesigntimer2('Cảnh báo','Chưa chọn phiếu.','warning',2000)");
		// }

		// lay nhung don hang nao co cap nhat
		try {
			List<InvoiceDMS> datas = invoiceDetailService.selectByInvoicesToDMS2(monthDms, yearDms);
			String result = CallAPI.sendInvoiceToDMS(datas, invoiceService, invoiceDetailService, myLogDMSService);
			success("Đồng bộ thành công, " + result);
			System.out.println("SL invoice gui di: " + datas.size());
		} catch (Exception e) {
			e.printStackTrace();
		}

		try {
			// Lay nhung don hang xoa
			List<String> invDeletes = invoiceDeleteService.findAllIdRefsByInvoiceDate(monthDms, yearDms);
			CallAPI.sendDeletionToDMS(invDeletes, invoiceService, myLogDMSService);
		} catch (Exception e) {
			e.printStackTrace();
		}
		/*
		 * Chuyển hệ qua hệ thống DMS phía nam
		 */
		try {
			List<InvoiceDMS> datas = invoiceDetailService.selectByInvoicesToDMS2MienBacNapVaoMienNam(monthDms, yearDms);
			if (datas != null) {
				String result = CallAPIMBNapVaoMN.sendInvoiceToDMS(datas, invoiceService, invoiceDetailService,
						myLogDMSService);
				success("Đồng bộ thành công mien nam, " + result);
				System.out.println("SL invoice mien nam gui di: " + datas.size());
			} else {
				System.out
						.println("KQ:invoiceDetailService.selectByInvoicesToDMS2MienBacNapVaoMienNam(monthDms, yearDms) => null");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			// Lay nhung don hang xoa
			List<String> invDeletes = invoiceDeleteService.findAllIdRefsByInvoiceDate(monthDms, yearDms);
			CallAPIMBNapVaoMN.sendDeletionToDMS(invDeletes, invoiceService, myLogDMSService);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	@Inject
	MyLogDMSService myLogDMSService;

	public String sendInvoiceToDMS(Invoice invoice) {
		try {
			List<Object[]> rows = invoiceDetailService.selectByInvoicesToDMS(invoice.getId());
			// return CallAPI.sendInvoiceToDMS(rows, invoiceService,
			// invoiceDetailService, myLogDMSService);
		} catch (Exception e) {
			logger.error("sendInvoiceToDMS error: " + e.getMessage(), e);
		}
		return "Lỗi không chuyển được";
	}

	public void exportCSVDMS2() {
		try {
			List<Long> idInvoices = new ArrayList<Long>();
			int size = listInvoice.size();
			for (int i = 0; i < size; i++) {
